...
(function () {
  ...
  var ALLOWED_TOOLSETS = new Set(['standard','premium','secure','governance','docforge']);
...
(function initAiChatWidget() {
  ...
  async function loadStatus() {
    try {
      const r = await fetch('/api/ai-chat/status', { headers: { Authorization: 'Bearer ' + (localStorage.getItem('pdfrealm_token') || '') } });
  ...
  clearBtn.addEventListener('click', async () => {
    await fetch('/api/ai-chat/clear', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + (localStorage.getItem('pdfrealm_token') || '') }, body: JSON.stringify({ sessionId }) });
  ...
  if (buyLink) {
    buyLink.addEventListener('click', async (e) => {
      e.preventDefault();
      const r = await fetch('/api/ai-chat/buy', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + (localStorage.getItem('pdfrealm_token') || '') }, body: JSON.stringify({ bundle: 'starter' }) });
