...
function getPpeForTool(tool) {
  // New model: single global 24h pass token takes priority
  const globalToken = localStorage.getItem("pdfrealm_ppe_token");
  if (globalToken) return globalToken;
  // Legacy per-tool fallback
  const m = getPpeMap();
  return m[tool] || "";
}

// Check if a valid global 24h pass is stored
function hasPpePass() {
  const token   = localStorage.getItem("pdfrealm_ppe_token");
  const expires = localStorage.getItem("pdfrealm_ppe_expires");
  if (!token) return false;
  if (expires && new Date(expires) < new Date()) {
    localStorage.removeItem("pdfrealm_ppe_token");
    localStorage.removeItem("pdfrealm_ppe_expires");
    return false;
  }
  return true;
}

// Kick off the $2.99 checkout
async function buyAllToolsPass() {
  try {
    const res  = await fetch("/api/pay-per-export/create-checkout-session", { method: "POST" });
...
// -------------------- Share Result Helper --------------------
/**
 * pdfrealmShareResult(blob, filename, expiryHours)
 * Uploads a blob to the vault, creates a secure share link, and shows a toast.
 * Gracefully fails (shows error toast) if the user is not logged in.
 */
async function pdfrealmShareResult(blob, filename, expiryHours = 24) {
  const token = (typeof getToken === "function") ? getToken() : null;
  if (!token) {
    pdfrealmShareToast(null, null, "Sign in to create share links.");
    return null;
  }

  try {
    pdfrealmShareToast(null, null, "Uploading…", true);

    // 1. Upload to vault
    const fd = new FormData();
    fd.append("file", blob, filename || "pdfrealm-result.pdf");
    ...
    if (!upRes.ok) {
      ...
      throw new Error(upErr.error || upErr.message || `Upload failed (${upRes.status})`);
    }
    const upData = await upRes.json();
    const objectId = upData.id || upData.fileId || upData.file_id;
    if (!objectId) throw new Error("Upload did not return a file ID.");

    // 2. Create secure share
    const expiresAt = new Date(Date.now() + expiryHours * 3600 * 1000).toISOString();
    const shareRes = await fetch("/api/secure-shares", {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
      body: JSON.stringify({ objectIds: [objectId], expiresAt, requirePasscode: false }),
    });
    if (!shareRes.ok) {
      const sErr = await shareRes.json().catch(() => ({}));
      throw new Error(sErr.error || sErr.message || `Share creation failed (${shareRes.status})`);
    }
    const shareData = await shareRes.json();
    const shareUrl = shareData.share_url || shareData.url || shareData.shareUrl || (shareData.token ? `${window.location.origin}/s/${shareData.token}` : null);
    const passcode = shareData.passcode || shareData.share_passcode || null;

    if (!shareUrl) throw new Error("Share link was not returned by the server.");

    pdfrealmShareToast(shareUrl, passcode, null, false);
    return shareUrl;
  } catch (e) {
    console.error("[pdfrealm] Share failed:", e);
    pdfrealmShareToast(null, null, "Share failed: " + (e.message || e));
    return null;
  }
}

function pdfrealmShareToast(url, passcode, errorMsg, loading) {
  // Remove any existing share toast
  const existing = document.getElementById("pdfr-share-toast");
  if (existing) existing.remove();

  const toast = document.createElement("div");
  toast.id = "pdfr-share-toast";
  toast.style.cssText = [
    "position:fixed", "bottom:24px", "right:24px", "z-index:99999",
    "background:var(--card-bg,#1e293b)", "border:1px solid var(--border,rgba(255,255,255,0.12))",
    "border-radius:10px", "padding:16px 18px", "min-width:280px", "max-width:420px",
    "box-shadow:0 8px 32px rgba(0,0,0,0.45)", "font-size:0.93rem", "color:var(--text,#f1f5f9)",
    "display:flex", "flex-direction:column", "gap:8px"
  ].join(";");

  if (loading) {
    toast.innerHTML = `<div style="color:var(--muted,#94a3b8);">⏳ Creating share link…</div>`;
  } else if (errorMsg) {
    toast.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
        <span style="color:var(--danger,#ef4444);">⚠️ ${errorMsg}</span>
        <button onclick="document.getElementById('pdfr-share-toast')?.remove()" style="background:none;border:none;color:var(--muted,#94a3b8);cursor:pointer;font-size:1.1rem;padding:0;">✕</button>
      </div>`;
  } else {
    const passHtml = passcode ? `<div style="margin-top:2px;font-size:0.85rem;color:var(--muted,#94a3b8);">Passcode: <strong>${passcode}</strong></div>` : "";
    toast.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
        <strong>📎 Share Link Ready</strong>
        <button onclick="document.getElementById('pdfr-share-toast')?.remove()" style="background:none;border:none;color:var(--muted,#94a3b8);cursor:pointer;font-size:1.1rem;padding:0;">✕</button>
      </div>
      <input id="pdfr-share-url-input" type="text" value="${url}" readonly style="width:100%;background:var(--input-bg,rgba(255,255,255,0.05));border:1px solid var(--border,rgba(255,255,255,0.12));border-radius:6px;padding:7px 10px;color:inherit;font-size:0.9rem;box-sizing:border-box;" />
      ${passHtml}
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button onclick="(()=>{const i=document.getElementById('pdfr-share-url-input');if(i){navigator.clipboard.writeText(i.value).then(()=>{this.textContent='✓ Copied!';setTimeout(()=>this.textContent='Copy Link',1500)})}})()" style="flex:1;padding:7px 12px;background:var(--primary,#6366f1);border:none;border-radius:6px;color:#fff;cursor:pointer;font-size:0.9rem;">Copy Link</button>
        <button onclick="document.getElementById('pdfr-share-toast')?.remove()" style="padding:7px 12px;background:var(--card-bg2,rgba(255,255,255,0.08));border:1px solid var(--border,rgba(255,255,255,0.12));border-radius:6px;color:inherit;cursor:pointer;font-size:0.9rem;">Close</button>
      </div>
      <div style="font-size:0.8rem;color:var(--muted,#94a3b8);">⏱ Link expires in 24 hours</div>`;
  }

  document.body.appendChild(toast);

  // Auto-dismiss after 30s (only for success and error, not loading)
  if (!loading) {
    setTimeout(() => { if (document.getElementById("pdfr-share-toast") === toast) toast.remove(); }, 30000);
  }
}

// -------------------- Save to Vault Helper --------------------
/**
 * pdfrealmSaveToVault(blob, filename)
 * Uploads an export blob to the user's vault and shows a toast.
 */
async function pdfrealmSaveToVault(blob, filename) {
  const token = (typeof getToken === "function") ? getToken() : null;
  if (!token) {
    pdfrealmSaveToVaultToast(false, "Sign in to save to Vault.");
    return;
  }
  pdfrealmSaveToVaultToast(null, null, true); // loading
  try {
    const fd = new FormData();
    fd.append("file", blob instanceof File ? blob : new File([blob], filename || "pdfrealm-export.pdf", { type: blob.type || "application/octet-stream" }));
    ...
    if (!res.ok) {
      ...
      throw new Error(d.error || d.message || `Upload failed (${res.status})`);
    }
    pdfrealmSaveToVaultToast(true, `Saved "${filename || "file"}" to Vault.`);
  } catch (e) {
    console.error("[pdfrealm] Save to Vault failed:", e);
    pdfrealmSaveToVaultToast(false, "Save to Vault failed: " + (e.message || e));
  }
}

function pdfrealmSaveToVaultToast(success, message, loading) {
  const existing = document.getElementById("pdfr-vault-toast");
  if (existing) existing.remove();
  const el = document.createElement("div");
  el.id = "pdfr-vault-toast";
  el.style.cssText = [
    "position:fixed","bottom:24px","left:24px","z-index:99999",
    "background:var(--card-bg,#1e293b)","border:1px solid var(--border,rgba(255,255,255,0.12))",
    "border-radius:10px","padding:14px 16px","min-width:240px","max-width:380px",
    "box-shadow:0 8px 32px rgba(0,0,0,0.45)","font-size:0.93rem","color:var(--text,#f1f5f9)",
    "display:flex","align-items:center","gap:10px"
  ].join(";");
  if (loading) {
    el.innerHTML = `<span>⏳ Saving to Vault…</span>`;
  } else {
    const icon = success ? "✅" : "⚠️";
    el.innerHTML = `
      <span>${icon} ${message || ""}</span>
      <button onclick="document.getElementById('pdfr-vault-toast')?.remove()" style="margin-left:auto;background:none;border:none;color:var(--muted,#94a3b8);cursor:pointer;font-size:1rem;padding:0;">✕</button>
      ${success ? `<a href="/vault.html" style="padding:6px 10px;background:var(--primary,#6366f1);border:none;border-radius:6px;color:#fff;cursor:pointer;font-size:0.88rem;text-decoration:none;white-space:nowrap;">Open Vault</a>` : ""}
    `;
  }
  document.body.appendChild(el);
  if (!loading) setTimeout(() => { if (document.getElementById("pdfr-vault-toast") === el) el.remove(); }, 7000);
}

/**
 * Shows a persistent "Save to Vault" button next to an existing download button/link.
 * Call after a successful export to wire up the button.
 */
function pdfrealmShowSaveToVaultBtn(blob, filename, anchorElOrId) {
  const anchor = typeof anchorElOrId === "string" ? document.getElementById(anchorElOrId) : anchorElOrId;
  const id = "pdfr-save-vault-btn-" + (filename || "file").replace(/[^a-z0-9]/gi, "_");
  ...
  btn.addEventListener("click", () => pdfrealmSaveToVault(blob, filename));
...
async function runPaidPdfExport({
  ...
  try {
    ...
    // Show floating "Save to Vault" button after successful export
    if (filename && blob && typeof pdfrealmSaveToVault === "function") {
      try {
        // Remove any previous save-to-vault bar
        const prev = document.getElementById("pdfr-vault-save-bar");
        if (prev) prev.remove();
        const bar = document.createElement("div");
        bar.id = "pdfr-vault-save-bar";
        bar.style.cssText = [
          "position:fixed","bottom:72px","right:24px","z-index:99998",
          "background:rgba(15,23,42,0.95)","border:1px solid rgba(96,165,250,0.4)",
          "border-radius:12px","padding:10px 14px",
          "display:flex","align-items:center","gap:10px",
          "box-shadow:0 8px 30px rgba(0,0,0,0.5)","font-size:0.9rem","color:#e2e8f0"
        ].join(";");
        const capturedBlob = blob;
        const capturedFilename = filename;
        bar.innerHTML = `
          <span style="color:#94a3b8;">Export ready:</span>
          <span style="font-weight:600;">${capturedFilename}</span>
          <button id="pdfr-vault-save-now" style="padding:6px 12px;background:#6366f1;border:none;border-radius:8px;color:#fff;cursor:pointer;font-weight:600;font-size:0.88rem;">💾 Save to Vault</button>
          <button onclick="document.getElementById('pdfr-vault-save-bar')?.remove()" style="background:none;border:none;color:#64748b;cursor:pointer;font-size:1rem;padding:0 4px;">✕</button>
        `;
        document.body.appendChild(bar);
        const saveNowBtn = document.getElementById("pdfr-vault-save-now");
        if (saveNowBtn) {
          saveNowBtn.addEventListener("click", async () => {
            saveNowBtn.disabled = true;
            saveNowBtn.textContent = "Saving…";
            await pdfrealmSaveToVault(capturedBlob, capturedFilename);
            const b = document.getElementById("pdfr-vault-save-bar");
            if (b) b.remove();
          });
        }
        setTimeout(() => { const b = document.getElementById("pdfr-vault-save-bar"); if (b) b.remove(); }, 30000);
      } catch (ignore) {}
    }
    ...
    if (e.status === 402) {
      // Show a clean paywall prompt — no account needed, just $2.99 for 24h all-tools pass
      const wantsPpe = confirm(
        "🔓 Unlock all tools for 24 hours — just $2.99.\n\nOK = Get access now (no account needed)\nCancel = Sign in / subscribe"
      );
      if (wantsPpe) {
        await buyAllToolsPass(); // redirects to Stripe
...
const PDFREALM_TIER = (() => {
  ...
  const PRICES = {
    standard: 2.99,  // all-tools 24h pass
    premium: 2.99,   // same pass covers everything
  };
  ...
  "secure_voip": "secure",
  "breakthrough": "secure",
  "docforge_pdf": "docforge",
  "docforge_docx": "docforge",
  "docforge_pptx": "docforge",
  "docforge_xlsx": "docforge",
  ...
  "xml2pdf": "standard",
  "zip2pdf": "standard",
"signreq_create": "signreqs",
"signreq_list": "signreqs",
"signreq_sign": "signreqs",
"signreq_proof": "signreqs"
};
  ...
  function applyTierFilter(tier) {
    ...
    tabs.forEach((t) => {
      ...
      const show = (tt === tier);
      // Use both `hidden` and display to ensure consistent behavior across CSS variants
      t.hidden = !show;
    ...
    // Ensure the visible list starts at the top when switching tiers
    try {
  ...
  function setTier(tier) {
    ...
    try { document.body && document.body.setAttribute('data-active-tier', safe); } catch {}
    setTierActiveButton(safe);
    applyTierFilter(safe);
  }

  function init() {
    if (window.__pdfrealmTierInited) return;
    window.__pdfrealmTierInited = true;

    // TOP_TOOLSET_MODE_SKIP_TIER_FILTER_V1:
    // If the top toolset bar exists, do NOT tier-filter the sidebar. The top toolset controls suite visibility.
    try {
      if (document.getElementById('toolsetTabs')) {
...
function initTabs() {
  if (window.__pdfrealmTabsInited) return;
  window.__pdfrealmTabsInited = true;
  const tabs = Array.from(document.querySelectorAll(".tool-tab"));
  const views = Array.from(document.querySelectorAll(".tool-view"));
  ...
  if (loginBtn) {
    loginBtn.addEventListener("click", async () => {
      try {
        ...
        let refCode = '';
        try { refCode = localStorage.getItem('pdfrealm_ref') || ''; } catch(e) {}
        ...
        setToken(out.token || "");
        if (out.token) {
          // Close the account modal after successful login
          try { closeModal("accountModal"); } catch {}
          if (status) status.textContent = "Logged in.";
          try { updateAccountDot(); } catch {}
          // Refresh vault if open
          try { await refreshVaultList(); } catch {}
          // Update AI chat credit display
          try {
            var aiStatusEl = document.getElementById('ai-chat-credit-display');
            if (aiStatusEl) aiStatusEl.textContent = 'Loading...';
            const cr = await fetch('/api/ai-chat/status', { headers: { Authorization: 'Bearer ' + out.token } });
            if (cr.ok) { const cd = await cr.json(); if (aiStatusEl) aiStatusEl.textContent = (cd.credits || 0) + ' credits remaining'; }
          } catch {}
        } else {
          if (status) status.textContent = "Login returned no token.";
        }
      } catch (e) {
      ...
      try {
        ...
        try {
          ...
          if (lib2 && lib2.GlobalWorkerOptions) {
            lib2.GlobalWorkerOptions.workerSrc = "/vendor/pdfjs/pdf.worker.min.js";
          }
          return lib2;
        } catch (e2) {
...
function initPassword() {
  ...
  encryptBtn.addEventListener('click', async () => {
    ...
    const passwordShareBtn = $('passwordShareBtn');
    if (passwordShareBtn) passwordShareBtn.style.display = 'none';

    const pwResult = await runPaidPdfExport({
      tool: 'encrypt',
    ...
    if (pwResult?.ok && pwResult?.blob && passwordShareBtn) {
      passwordShareBtn.style.display = '';
      passwordShareBtn._lastBlob = pwResult.blob;
      passwordShareBtn._lastFilename = 'pdfrealm-protected.pdf';
    }
  });

  // Wire share button for password/encrypt
  const passwordShareBtnEl = $('passwordShareBtn');
  if (passwordShareBtnEl) {
    passwordShareBtnEl.style.display = 'none';
    passwordShareBtnEl.addEventListener('click', () => {
      const b = passwordShareBtnEl._lastBlob;
      if (b) pdfrealmShareResult(b, passwordShareBtnEl._lastFilename || 'pdfrealm-protected.pdf');
      else alert('Export a protected PDF first, then click Share Link.');
    });
  }
...
// -------------------- Merge / Delete / Split --------------------
function initMerge() {
  // --- PDFRealm Merge Preview helpers (handshake-safe) ---
  function __pdfrealmPostMergePreview(frame, topUrl, bottomUrl) {
    if (!frame) return;
    const payload = { type: "pdfrealm:mergePreview", topUrl: topUrl || "", bottomUrl: bottomUrl || "" };
    try { frame.contentWindow && frame.contentWindow.postMessage(payload, window.location.origin); } catch {}
  }
  function __pdfrealmEnsureMergeShell(frame, topUrl, bottomUrl) {
    if (!frame) return;
    frame.dataset.topUrl = topUrl || "";
    frame.dataset.bottomUrl = bottomUrl || "";

    const cur = frame.getAttribute("src") || "";
    if (!cur.includes("merge_preview_shell.html")) frame.setAttribute("src", "/merge_preview_shell.html");

    const send = () => __pdfrealmPostMergePreview(frame, frame.dataset.topUrl, frame.dataset.bottomUrl);
    ...
    if (!window.__pdfrealmMergePreviewReadyListener) {
      window.__pdfrealmMergePreviewReadyListener = true;
      window.addEventListener("message", (ev) => {
        if (ev.origin !== window.location.origin) return;
        if (!ev.data || ev.data.type !== "pdfrealm:mergePreviewReady") return;
        const f = document.querySelector("#mergePreviewFrame") || document.querySelector("[data-merge-preview]") || document.querySelector("iframe.mergePreview");
        if (!f) return;
        __pdfrealmPostMergePreview(f, f.dataset.topUrl || "", f.dataset.bottomUrl || "");
      });
    }
  }
  // --- end helpers ---
  ...
  // --- Merge preview (stable shell) ---
  function __mergeSendToShell(topUrl, bottomUrl) {
    const frame = $(frameId);
    if (!frame) return;
    ...
    const ensureSrc = () => {
      const cur = frame.getAttribute("src") || "";
      if (!cur.includes("merge_preview_shell.html")) {
        frame.setAttribute("src", "/merge_preview_shell.html");
      }
    };
    ...
    const send = () => {
      try {
        frame.contentWindow && frame.contentWindow.postMessage(
          { type: "pdfrealm:mergePreview", topUrl: frame.dataset.topUrl || "", bottomUrl: frame.dataset.bottomUrl || "" },
    ...
    // Send after load + retry (covers races)
    frame.addEventListener("load", () => send(), { once: true });
    setTimeout(send, 60);
    setTimeout(send, 250);

    // Re-send when shell announces readiness
    if (!window.__pdfrealmMergePreviewReadyListener) {
      window.__pdfrealmMergePreviewReadyListener = true;
      window.addEventListener("message", (ev) => {
        if (ev.origin !== window.location.origin) return;
        ...
        try {
          f.contentWindow && f.contentWindow.postMessage(
            { type: "pdfrealm:mergePreview", topUrl: f.dataset.topUrl || "", bottomUrl: f.dataset.bottomUrl || "" },
            window.location.origin
          );
        } catch {}
      });
  ...
  // --- Merge preview (stable shell + sane encryption gating) ---
  function __mergeEnsureShell(frame) {
    if (!frame) return;
    const cur = frame.getAttribute("src") || "";
    if (!cur.includes("merge_preview_shell.html")) frame.setAttribute("src", "/merge_preview_shell.html");
  }

  function __mergePost(frame, topUrl, bottomUrl) {
    if (!frame) return;
    frame.dataset.topUrl = topUrl || "";
    frame.dataset.bottomUrl = bottomUrl || "";
    const send = () => {
  ...
  function updatePreview(forceDisplay = false) {
    const frame = $(frameId);
    const ph = $(placeholderId);
    ...
    if (frame) __mergeEnsureShell(frame);

    revokePreviewUrls();

    if (!f1 && !f2) {
      if (ph) ph.textContent = "Select PDFs to preview them together.";
      showPlaceholder(placeholderId, true);
      if (frame) __mergePost(frame, "", "");
      return;
    }

    // Only block when we KNOW it's encrypted (encX === true)
    const topBlocked = !!(f1 && enc1 === true && !unlocked1);
    const botBlocked = !!(f2 && enc2 === true && !unlocked2);

    if (topBlocked || botBlocked) {
      const parts = [];
      if (topBlocked) parts.push("Top PDF is encrypted — enter password then Decrypt / Unlock");
      if (botBlocked) parts.push("Bottom PDF is encrypted — enter password then Decrypt / Unlock");
    ...
    const d1 = unlocked1 || f1;
    const d2 = unlocked2 || f2;

    const topUrl = d1 ? URL.createObjectURL(d1) : "";
    const botUrl = d2 ? URL.createObjectURL(d2) : "";
    innerPdfUrls = [topUrl, botUrl].filter(Boolean);

    showPlaceholder(placeholderId, false);
    if (frame) __mergePost(frame, topUrl, botUrl);
  }
  // --- end merge preview ---
  ...
  btn.addEventListener("click", async () => {
    ...
    const mergeShareBtn = $('mergeShareBtn');
    if (mergeShareBtn) mergeShareBtn.style.display = 'none';

    const mergeResult = await runPaidPdfExport({
      tool: "merge",
    ...
    if (mergeResult?.ok && mergeResult?.blob && mergeShareBtn) {
      mergeShareBtn.style.display = '';
      mergeShareBtn._lastBlob = mergeResult.blob;
      mergeShareBtn._lastFilename = 'pdfrealm-merged.pdf';
    }
  });

  // Wire share button for merge
  const mergeShareBtnEl = $('mergeShareBtn');
  if (mergeShareBtnEl) {
    mergeShareBtnEl.style.display = 'none';
    mergeShareBtnEl.addEventListener('click', () => {
      const b = mergeShareBtnEl._lastBlob;
      if (b) pdfrealmShareResult(b, mergeShareBtnEl._lastFilename || 'pdfrealm-merged.pdf');
      else alert('Export a merged PDF first, then click Share Link.');
    });
  }
...
/* PDFREALM_TOOLSET_DISABLE_LEGACY_V1
   We now use a single toolset controller (PDFREALM_TOOLSET_MASTER_V1).
*/
try{
  window.__PDFREALM_TOOLSET_NAV_V3__ = true;
  window.__PDFREALM_TOOLSET_DROPDOWN_NAV_V1__ = true;
  window.__PDFREALM_TOOLSET_TABS_SWITCHER_V2__ = true;
} catch {}


/* PDFREALM_DISABLE_TIER_PAGER_FOR_TOP_TOOLSET_V1 */
try {
  window.__PDFREALM_TIER_PAGER_V4__ = true;
  window.__PDFREALM_TIER_PAGER_V3__ = true;
  window.__PDFREALM_TIER_PAGER_V2__ = true;
  window.__PDFREALM_TIER_PAGER_V1__ = true;
} catch (_) {}
...
function initSplit() {
  ...
  btn.addEventListener("click", async () => {
    ...
    if (mode === "single") {
      ...
      if (out?.ok && out.objectUrl && out.blob) {
        ...
        // Show share button
        const splitShareBtn = $('splitShareBtn');
        if (splitShareBtn) {
          splitShareBtn.style.display = '';
          splitShareBtn._lastBlob = out.blob;
          splitShareBtn._lastFilename = base + '_extract.pdf';
        }
      }
      return;
    }
  ...
  // Wire share button for split
  const splitShareBtnEl = $('splitShareBtn');
  if (splitShareBtnEl) {
    splitShareBtnEl.style.display = 'none';
    splitShareBtnEl.addEventListener('click', () => {
      const b = splitShareBtnEl._lastBlob;
      if (b) pdfrealmShareResult(b, splitShareBtnEl._lastFilename || 'pdfrealm-split.pdf');
      else alert('Export a split PDF first, then click Share Link.');
    });
  }
...
function initReorderSecure() {
  ...
  exportBtn.addEventListener('click', async () => {
    ...
    const reorderShareBtn = $('reorderShareBtn');
    if (reorderShareBtn) reorderShareBtn.style.display = 'none';
    ...
    if (out?.ok) {
      ...
      updatePreviewSyncUI();
      if (reorderShareBtn && out.blob) {
        reorderShareBtn.style.display = '';
        reorderShareBtn._lastBlob = out.blob;
        reorderShareBtn._lastFilename = 'pdfrealm-reordered.pdf';
      }
    }
  });

  // Wire share button for reorder
  const reorderShareBtnEl = $('reorderShareBtn');
  if (reorderShareBtnEl) {
    reorderShareBtnEl.style.display = 'none';
    reorderShareBtnEl.addEventListener('click', () => {
      const b = reorderShareBtnEl._lastBlob;
      if (b) pdfrealmShareResult(b, reorderShareBtnEl._lastFilename || 'pdfrealm-reordered.pdf');
      else alert('Export a reordered PDF first, then click Share Link.');
    });
  }
...
function initRotateInteractive() {
  ...
  exportBtn.addEventListener("click", async () => {
    ...
    const rotateShareBtn = $('rotateShareBtn');
    if (rotateShareBtn) rotateShareBtn.style.display = 'none';

    const rotResult = await runPaidPdfExport({
      tool: "rotate",
    ...
    if (rotResult?.ok && rotResult?.blob && rotateShareBtn) {
      rotateShareBtn.style.display = '';
      rotateShareBtn._lastBlob = rotResult.blob;
      rotateShareBtn._lastFilename = 'pdfrealm-rotated.pdf';
    }
  });

  // Wire share button for rotate
  const rotateShareBtnEl = $('rotateShareBtn');
  if (rotateShareBtnEl) {
    rotateShareBtnEl.style.display = 'none';
    rotateShareBtnEl.addEventListener('click', () => {
      const b = rotateShareBtnEl._lastBlob;
      if (b) pdfrealmShareResult(b, rotateShareBtnEl._lastFilename || 'pdfrealm-rotated.pdf');
      else alert('Export a rotated PDF first, then click Share Link.');
    });
  }
...
function initSinglePdfToolWithUnlock({
  ...
  const shareBtn = shareBtnId ? $(shareBtnId) : null;
  let _lastShareBlob = null;
  let _lastShareFilename = null;
  ...
  actionBtn.addEventListener('click', async () => {
    ...
    // Hide share button while running
    if (shareBtn) shareBtn.style.display = 'none';
    ...
    if (result?.ok && result?.blob && shareBtn) {
      _lastShareBlob = result.blob;
      _lastShareFilename = filename || 'pdfrealm-result.pdf';
  ...
  if (shareBtn) {
    shareBtn.addEventListener('click', () => {
      if (_lastShareBlob) pdfrealmShareResult(_lastShareBlob, _lastShareFilename);
      else alert('Export a result first, then click Share Link.');
    });
  }
...
function initMetadataSecure() {
  ...
  const metaShareBtn = $('metaShareBtn');
  if (metaShareBtn) metaShareBtn.style.display = 'none';
  ...
  exportBtn.addEventListener('click', () => {
    ...
    // Show share button after download
    if (metaShareBtn) {
      metaShareBtn.style.display = '';
      metaShareBtn._lastBlob = lastBlob;
      metaShareBtn._lastFilename = lastFilename;
    }
  });

  if (metaShareBtn) {
    metaShareBtn.addEventListener('click', () => {
      const b = metaShareBtn._lastBlob;
      if (b) pdfrealmShareResult(b, metaShareBtn._lastFilename || 'pdfrealm-metadata.pdf');
      else alert('Export a PDF first, then click Share Link.');
    });
  }
...
function initRedactInteractive() {
  ...
  async function onExport() {
    ...
    // Show share button now that we have a result
    try {
      const redactShareBtn = $('redactShareBtn');
      if (redactShareBtn && result.blob) {
        redactShareBtn.style.display = '';
        redactShareBtn._lastBlob = result.blob;
        redactShareBtn._lastFilename = 'pdfrealm-redacted.pdf';
      }
    } catch (_) {}
  ...
  // Share button for redact result
  const redactShareBtnEl = $('redactShareBtn');
  if (redactShareBtnEl) {
    redactShareBtnEl.style.display = 'none';
    redactShareBtnEl.addEventListener('click', () => {
      const b = redactShareBtnEl._lastBlob;
      const fn = redactShareBtnEl._lastFilename || 'pdfrealm-redacted.pdf';
      if (b) pdfrealmShareResult(b, fn);
      else alert('Export a redacted PDF first, then click Share Link.');
    });
  }
...
function initOcr() {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      const ocrPpe = typeof getPpeForTool === 'function' ? getPpeForTool('ocr') : '';
      if (ocrToken) ocrHeaders['Authorization'] = 'Bearer ' + ocrToken;
      if (ocrPpe) ocrHeaders['x-ppe-session'] = ocrPpe;
      const res = await fetch(ENDPOINTS.OCR, { method: 'POST', headers: ocrHeaders, body: fd });
      const data = await res.json().catch(() => null);
      if (!res.ok) { if (res.status === 402 && typeof showPaywallModal === 'function') { showPaywallModal('ocr'); return; } throw new Error(data?.error || 'OCR failed'); }
...
// ============================================================
// SIGN REQUESTS FRONTEND
// ============================================================
function initSignRequests() {
  var _signToken = null;

  function statusBadge(status) {
    var colors = { draft:'#6b7280', sent:'#2563eb', viewed:'#7c3aed', completed:'#16a34a', declined:'#dc2626' };
    var color = colors[status] || '#6b7280';
    return '<span style="background:'+color+';color:#fff;padding:2px 8px;border-radius:12px;font-size:0.78rem;">'+status+'</span>';
  }
  ...
  // File preview
  var sreqFile = document.getElementById('sreqFile');
  var sreqFilePreview = document.getElementById('sreqFilePreview');
  if (sreqFile && sreqFilePreview) {
    sreqFile.addEventListener('change', function() {
      var f = sreqFile.files && sreqFile.files[0];
      if (f) {
        var kb = (f.size / 1024).toFixed(1);
        sreqFilePreview.textContent = f.name + ' (' + kb + ' KB)';
      } else {
        sreqFilePreview.textContent = '';
      }
    });
  }

  // Recipient count
  var sreqRecipients = document.getElementById('sreqRecipients');
  var sreqRecipientCount = document.getElementById('sreqRecipientCount');
  if (sreqRecipients && sreqRecipientCount) {
    sreqRecipients.addEventListener('input', function() {
      var lines = sreqRecipients.value.split('\n').filter(function(l){ return l.includes('@'); });
      sreqRecipientCount.textContent = lines.length + ' recipient' + (lines.length === 1 ? '' : 's') + ' detected';
    });
  }

  // Create + send
  var sreqCreateBtn = document.getElementById('sreqCreateBtn');
  var sreqStatus = document.getElementById('sreqStatus');
  if (sreqCreateBtn) {
    sreqCreateBtn.addEventListener('click', async function() {
      var fileInput = document.getElementById('sreqFile');
      var f = fileInput && fileInput.files && fileInput.files[0];
      if (!f) { sreqStatus.textContent = 'Please select a PDF file.'; return; }
      sreqCreateBtn.disabled = true;
      sreqStatus.textContent = 'Uploading...';
      try {
        var fd = new FormData();
        fd.append('pdf', f);
        fd.append('title', document.getElementById('sreqTitle').value || f.name);
        fd.append('recipients', document.getElementById('sreqRecipients').value || '');
        fd.append('requesterName', document.getElementById('sreqRequesterName').value || '');
        fd.append('requesterEmail', document.getElementById('sreqRequesterEmail').value || '');
        fd.append('message', document.getElementById('sreqMessage').value || '');
        fd.append('expiresInDays', document.getElementById('sreqExpires').value || '7');
        var r = await fetch('/api/sign-requests/create', { method:'POST', body: fd });
        var data = await r.json();
        if (!data.ok) { sreqStatus.textContent = 'Error: ' + (data.error || 'Unknown error'); sreqCreateBtn.disabled = false; return; }
        sreqStatus.textContent = 'Created! Sending invite to first signer...';
        var r2 = await fetch('/api/sign-requests/' + data.id + '/send', { method:'POST', headers:{'Content-Type':'application/json'}, body:'{}' });
        var d2 = await r2.json();
        if (d2.ok) {
          sreqStatus.innerHTML = '✅ Request created and sent to <b>' + (d2.sentTo || 'first recipient') + '</b>. Request ID: ' + data.id;
        } else {
          sreqStatus.textContent = '✅ Request created (ID: ' + data.id + '). ' + (d2.message || 'Could not send invite.');
        }
        // Reset form
        document.getElementById('sreqTitle').value = '';
        fileInput.value = '';
        document.getElementById('sreqRecipients').value = '';
        document.getElementById('sreqMessage').value = '';
        if (sreqFilePreview) sreqFilePreview.textContent = '';
        if (sreqRecipientCount) sreqRecipientCount.textContent = '';
      } catch(e) {
        sreqStatus.textContent = 'Error: ' + e.message;
      }
      sreqCreateBtn.disabled = false;
    });
  }

  // List tab
  async function loadSignRequestList() {
    var container = document.getElementById('sreqListContainer');
    if (!container) return;
    container.innerHTML = '<div class="muted">Loading...</div>';
    try {
      var r = await fetch('/api/sign-requests');
      var data = await r.json();
      if (!data.ok || !data.requests || !data.requests.length) {
        container.innerHTML = '<div class="muted">No requests yet. Create one using "Request Signatures".</div>';
        return;
      }
      var html = '<table style="width:100%;border-collapse:collapse;font-size:0.9rem;">';
      html += '<thead><tr style="border-bottom:1px solid #e5e7eb;"><th style="text-align:left;padding:8px 4px;">Title</th><th>Status</th><th>Progress</th><th>Created</th><th>Actions</th></tr></thead><tbody>';
      data.requests.forEach(function(req) {
        var signed = parseInt(req.signed_count) || 0;
        var total = parseInt(req.recipient_count) || 0;
        var date = req.created_at ? new Date(req.created_at).toLocaleDateString() : '';
        html += '<tr style="border-bottom:1px solid #f3f4f6;">';
        html += '<td style="padding:8px 4px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="'+req.title+'">'+req.title+'</td>';
        html += '<td style="padding:8px 4px;text-align:center;">'+statusBadge(req.status)+'</td>';
        html += '<td style="padding:8px 4px;text-align:center;">'+signed+'/'+total+' signed</td>';
        html += '<td style="padding:8px 4px;text-align:center;color:#6b7280;font-size:0.82rem;">'+date+'</td>';
        html += '<td style="padding:8px 4px;text-align:center;">';
        if (req.status === 'sent' || req.status === 'viewed') {
          html += '<button class="btn btn-secondary" style="font-size:0.8rem;padding:4px 8px;margin-right:4px;" onclick="__sreqRemind(\''+req.id+'\')">Remind</button>';
        }
        html += '</td></tr>';
      });
      html += '</tbody></table>';
      ...
      container.innerHTML = '<div class="muted">Error loading requests: ' + e.message + '</div>';
    }
  }

  window.__sreqRemind = async function(id) {
    try {
      var r = await fetch('/api/sign-requests/' + id + '/remind', { method:'POST', headers:{'Content-Type':'application/json'}, body:'{}' });
      var data = await r.json();
      alert(data.ok ? 'Reminder sent to ' + data.remindedEmail : (data.message || data.error || 'Could not send reminder'));
    } catch(e) { alert('Error: ' + e.message); }
  };

  var sreqRefreshBtn = document.getElementById('sreqRefreshBtn');
  if (sreqRefreshBtn) sreqRefreshBtn.addEventListener('click', loadSignRequestList);

  // Load list when tab is activated
  document.addEventListener('click', function(e) {
    var t = e.target;
    if (t && t.classList && t.classList.contains('tool-tab') && t.getAttribute('data-tool') === 'signreq_list') {
      setTimeout(loadSignRequestList, 100);
    }
    if (t && t.id === 'signReqsToolsetBtn') {
      // activate Sign Requests tier
      var tierBtn = document.querySelector('.tier-tab[data-tier="signreqs"]');
      if (tierBtn) tierBtn.click();
    }
  });

  // Sign tab - load document
  var sreqLoadSignBtn = document.getElementById('sreqLoadSignBtn');
  var sreqSignStatus = document.getElementById('sreqSignStatus');
  var sreqSignPanel = document.getElementById('sreqSignPanel');
  if (sreqLoadSignBtn) {
    sreqLoadSignBtn.addEventListener('click', async function() {
      var input = document.getElementById('sreqSignToken').value;
      var token = extractToken(input);
      if (!token) { sreqSignStatus.textContent = 'Please enter a signing link or token.'; return; }
      sreqSignStatus.textContent = 'Loading...';
      sreqSignPanel.style.display = 'none';
      try {
        var r = await fetch('/api/sign-token/' + encodeURIComponent(token));
        var data = await r.json();
        if (data.alreadyCompleted) { sreqSignStatus.textContent = '✅ This document has already been fully signed: ' + data.title; return; }
        if (data.alreadySigned) { sreqSignStatus.textContent = '✅ You have already signed this document: ' + data.title; return; }
        if (data.alreadyDeclined) { sreqSignStatus.textContent = 'You have already declined to sign this document.'; return; }
        if (!data.ok) { sreqSignStatus.textContent = 'Error: ' + (data.error || 'Could not load document'); return; }
        _signToken = token;
        document.getElementById('sreqSignDocTitle').textContent = data.title || 'Untitled';
        document.getElementById('sreqSignDocInfo').textContent = (data.requesterName ? 'From: ' + data.requesterName + '  ' : '') + (data.expiresAt ? 'Expires: ' + new Date(data.expiresAt).toLocaleDateString() : '');
        sreqSignStatus.textContent = '';
        sreqSignPanel.style.display = '';
        document.getElementById('sreqSignResult').textContent = '';
        document.getElementById('sreqSignerName').value = '';
        document.getElementById('sreqConsent').checked = false;
      } catch(e) {
        sreqSignStatus.textContent = 'Error: ' + e.message;
      }
    });
  }

  // Submit signature
  var sreqSubmitSignBtn = document.getElementById('sreqSubmitSignBtn');
  if (sreqSubmitSignBtn) {
    sreqSubmitSignBtn.addEventListener('click', async function() {
      if (!_signToken) return;
      var signerName = document.getElementById('sreqSignerName').value.trim();
      var consent = document.getElementById('sreqConsent').checked;
      var resultEl = document.getElementById('sreqSignResult');
      if (!signerName) { resultEl.textContent = 'Please enter your name.'; return; }
      if (!consent) { resultEl.textContent = 'Please check the consent checkbox.'; return; }
      sreqSubmitSignBtn.disabled = true;
      resultEl.textContent = 'Signing...';
      try {
        var r = await fetch('/api/sign-token/' + encodeURIComponent(_signToken), {
          method: 'POST', headers: {'Content-Type':'application/json'},
          body: JSON.stringify({ signerName: signerName })
        });
        var data = await r.json();
        if (data.ok) {
          var msg = data.completed ? '✅ Document fully signed! ' : '✅ Signature applied! ';
          if (data.downloadUrl) msg += '<a href="' + data.downloadUrl + '" target="_blank" class="btn btn-secondary" style="margin-left:8px;">Download Signed PDF</a>';
          resultEl.innerHTML = msg;
          sreqSignPanel.style.display = 'none';
          _signToken = null;
        } else {
          resultEl.textContent = 'Error: ' + (data.error || 'Signing failed');
        }
      } catch(e) {
        resultEl.textContent = 'Error: ' + e.message;
      }
      sreqSubmitSignBtn.disabled = false;
    });
  }

  // Decline
  var sreqDeclineBtn = document.getElementById('sreqDeclineBtn');
  if (sreqDeclineBtn) {
    sreqDeclineBtn.addEventListener('click', async function() {
      if (!_signToken) return;
      var reason = prompt('Reason for declining (optional):') || '';
      if (!confirm('Are you sure you want to decline to sign this document?')) return;
      var resultEl = document.getElementById('sreqSignResult');
      try {
        var r = await fetch('/api/sign-token/' + encodeURIComponent(_signToken) + '/decline', {
          method: 'POST', headers: {'Content-Type':'application/json'},
          body: JSON.stringify({ reason: reason })
        });
        var data = await r.json();
        resultEl.textContent = data.ok ? '❌ You have declined to sign this document.' : ('Error: ' + (data.error || 'Failed'));
        sreqSignPanel.style.display = 'none';
        _signToken = null;
      } catch(e) {
        resultEl.textContent = 'Error: ' + e.message;
  ...
  // Proof viewer
  var sreqLoadProofBtn = document.getElementById('sreqLoadProofBtn');
  if (sreqLoadProofBtn) {
    sreqLoadProofBtn.addEventListener('click', async function() {
      var input = document.getElementById('sreqProofToken').value;
      var token = extractToken(input);
      var resultEl = document.getElementById('sreqProofResult');
      if (!token) { resultEl.textContent = 'Please enter a proof token or URL.'; return; }
      resultEl.innerHTML = '<div class="muted">Loading...</div>';
      try {
        var r = await fetch('/api/sign-proof/' + encodeURIComponent(token));
        var data = await r.json();
        if (!data.ok) { resultEl.textContent = 'Error: ' + (data.error || 'Not found'); return; }
        var html = '<div style="border:1px solid #e5e7eb;border-radius:12px;padding:16px;background:#fff;">';
        html += '<h3 style="margin:0 0 8px 0;">'+data.title+'</h3>';
        html += '<div style="margin-bottom:12px;">Status: '+statusBadge(data.status)+'</div>';
        if (data.originalSha256) html += '<div style="font-size:0.8rem;color:#6b7280;margin-bottom:4px;word-break:break-all;"><b>Original SHA256:</b> '+data.originalSha256+'</div>';
        if (data.signedSha256) html += '<div style="font-size:0.8rem;color:#6b7280;margin-bottom:12px;word-break:break-all;"><b>Signed SHA256:</b> '+data.signedSha256+'</div>';
        html += '<b>Recipients</b><table style="width:100%;border-collapse:collapse;font-size:0.85rem;margin-top:8px;">';
        html += '<thead><tr style="border-bottom:1px solid #e5e7eb;"><th style="text-align:left;padding:4px;">Email</th><th>Status</th><th>Signed</th></tr></thead><tbody>';
        (data.recipients || []).forEach(function(rec) {
          html += '<tr style="border-bottom:1px solid #f9fafb;"><td style="padding:4px;">'+(rec.email||'')+'</td><td style="text-align:center;">'+statusBadge(rec.status)+'</td><td style="text-align:center;font-size:0.8rem;color:#6b7280;">'+(rec.signed_at ? new Date(rec.signed_at).toLocaleString() : '-')+'</td></tr>';
        });
        html += '</tbody></table>';
        if (data.events && data.events.length) {
          html += '<b style="display:block;margin-top:16px;margin-bottom:8px;">Event Timeline</b>';
          data.events.forEach(function(ev) {
            html += '<div style="font-size:0.8rem;padding:3px 0;border-bottom:1px solid #f9fafb;"><span style="color:#6b7280;">'+new Date(ev.created_at).toLocaleString()+'</span> — <b>'+ev.kind+'</b></div>';
          });
        }
        html += '</div>';
  ...
  // Auto-load sign doc if ?token= in URL
  try {
    var urlParams = new URLSearchParams(window.location.search);
    var signTokenParam = urlParams.get('token');
    if (signTokenParam && window.location.pathname === '/sign-request') {
      var tierBtn = document.querySelector('.tier-tab[data-tier="signreqs"]');
      if (tierBtn) tierBtn.click();
      setTimeout(function() {
        var toolBtn = document.querySelector('.tool-tab[data-tool="signreq_sign"]');
        if (toolBtn) toolBtn.click();
        setTimeout(function() {
          var tokenInput = document.getElementById('sreqSignToken');
          if (tokenInput) { tokenInput.value = signTokenParam; document.getElementById('sreqLoadSignBtn') && document.getElementById('sreqLoadSignBtn').click(); }
        }, 200);
      }, 300);
    }
    var proofTokenParam = urlParams.get('proof') || (window.location.pathname.match(/\/proof\/(.+)/) || [])[1];
    if (proofTokenParam) {
      var tierBtn2 = document.querySelector('.tier-tab[data-tier="signreqs"]');
      if (tierBtn2) tierBtn2.click();
      setTimeout(function() {
        var toolBtn2 = document.querySelector('.tool-tab[data-tool="signreq_proof"]');
        if (toolBtn2) toolBtn2.click();
        setTimeout(function() {
          var proofInput = document.getElementById('sreqProofToken');
          if (proofInput) { proofInput.value = proofTokenParam; document.getElementById('sreqLoadProofBtn') && document.getElementById('sreqLoadProofBtn').click(); }
        }, 200);
      }, 300);
    }
  } catch(_) {}
...
__safeInit("initSignRequests", initSignRequests);
// ============================================================
// END SIGN REQUESTS FRONTEND
// ============================================================
...
function initQuickSecureChat() {
  ...
  let chatAiAddon = { stop: () => {}, stopAndFinalize: async () => {} };
  try {
    chatAiAddon = SECURE_AI_NOTES.attachChatAddon({
    ...
    console.warn("[pdfrealm] Chat AI addon unavailable:", e);
    if (qtChatAiToggle) qtChatAiToggle.style.display = "none";
    if (qtChatAiFinalizeBtn) qtChatAiFinalizeBtn.style.display = "none";
    if (qtChatAiDownloadBtn) qtChatAiDownloadBtn.style.display = "none";
    if (qtChatAiDeleteBtn) qtChatAiDeleteBtn.style.display = "none";
  }
  // /PDFREALM_SECURE_AI_NOTES_ASSISTANT_V1
...
function initQuickSecureVideo() {
  ...
  let videoAiAddon = { stop: () => {}, stopAndFinalize: async () => {} };
  try {
    videoAiAddon = SECURE_AI_NOTES.attachAudioAddon({
      sessionType: "video",
    ...
    console.warn("[pdfrealm] Video AI addon unavailable:", e);
    if (qtVideoAiToggle) qtVideoAiToggle.style.display = "none";
    if (qtVideoAiFinalizeBtn) qtVideoAiFinalizeBtn.style.display = "none";
    if (qtVideoAiDownloadBtn) qtVideoAiDownloadBtn.style.display = "none";
    if (qtVideoAiDeleteBtn) qtVideoAiDeleteBtn.style.display = "none";
  }
  // /PDFREALM_SECURE_AI_NOTES_ASSISTANT_V1
...
function initQuickSecureVoip() {
  ...
  let voipAiAddon = { stop: () => {}, stopAndFinalize: async () => {} };
  try {
    voipAiAddon = SECURE_AI_NOTES.attachAudioAddon({
      sessionType: "voip",
      getContextId: () => state.roomId,
      getIsHost: () => state.isHost,
      getCall: () => state.call,
    ...
    console.warn("[pdfrealm] VoIP AI addon unavailable:", e);
    if (qtVoipAiToggle) qtVoipAiToggle.style.display = "none";
    if (qtVoipAiFinalizeBtn) qtVoipAiFinalizeBtn.style.display = "none";
    if (qtVoipAiDownloadBtn) qtVoipAiDownloadBtn.style.display = "none";
    if (qtVoipAiDeleteBtn) qtVoipAiDeleteBtn.style.display = "none";
  }
  // /PDFREALM_SECURE_AI_NOTES_ASSISTANT_V1
...
(function(){
  ...
  const CDN_PDFJS  = "/vendor/pdfjs/pdf.min.js";
...
(function initBreakthroughRoadmap(){
  ...
  try { window.__pdfrealmBTActivate = activate; } catch (_) {}
  ...
  function wire(){
    ...
    // default: DO NOT auto-open Hash Evidence Machine
    // If markup already marks a tab active, respect it; otherwise keep blank until user clicks.
    const pre = view.querySelector('.bt-tab.bt-active[data-bt]');
    if (pre) activate(pre.getAttribute('data-bt'));
...
/* PDFREALM_HEADER_TOOLSET_TABS_V1_REMOVED_BY_FIX */



/* =================== PDFREALM_TOP_TOOLSET_CONTROLLER_V3 ===================
   Header suite tabs controller (replaces V2).
   - Click suite tab => set active toolset + open dropdown
   - Dropdown items built from left sidebar tool list
   - DocForge suite routes to Office Workspace tools
*/
(function () {
  'use strict';

  if (window.__PDFREALM_TOP_TOOLSET_CONTROLLER_V3__) return;
  window.__PDFREALM_TOP_TOOLSET_CONTROLLER_V3__ = true;

  // Disable older toolset scripts if present.
  try {
    window.__PDFREALM_HEADER_TOOLSET_TABS_V1__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V1__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V2__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V3__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V4__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V5__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V6__ = true;
    window.__PDFREALM_TOOLSET_NAV_PATCH_V7__ = true;
  } catch (_) {}

  var KEY = 'pdfrealm_active_toolset';
  var ALLOWED_TOOLSETS = new Set(['standard','premium','secure','governance','docforge','signreqs']);

  var SECURE_IDS = new Set(['secure_send','secure_chat','secure_video','secure_voip','secure_containers']);
  var GOV_ID = 'breakthrough';

  

  /* GOV_MODULE_SIDEBAR_INJECT_V1
     Creates governance module buttons in the LEFT pane (and hides the base breakthrough tab).
  */
  var GOV_MODULES = [
    { bt: 'hash',          label: 'Hash Evidence Machine' },
    { bt: 'timetravel',    label: 'PDF Time Travel™' },
    { bt: 'interrogation', label: 'PDF Interrogation Mode™' },
    { bt: 'constitution',  label: 'Document Constitution' },
    { bt: 'sentience',     label: 'PDF Sentience Mode™' },
    { bt: 'immortal',      label: 'Immortal Documents™' },
    { bt: 'moat',          label: 'Evidence Graph & API' }
  ];

  function injectGovModules(){
    try {
      var list = document.querySelector('.tool-tabs');
      if (!list) return;

      if (list.querySelector('.tool-tab[data-gov-module="1"]')) return;

      var ref = list.querySelector('.tool-tab[data-tool="' + GOV_ID + '"]:not([data-bt])');

      for (var i = GOV_MODULES.length - 1; i >= 0; i--) {
        var m = GOV_MODULES[i];
        var b = document.createElement('button');
        b.type = 'button';
        b.className = 'tool-tab';
        b.setAttribute('data-tool', GOV_ID);
        b.setAttribute('data-bt', m.bt);
        b.setAttribute('data-gov-module', '1');
...
var DOCFORGE_IDS = new Set(['docforge_pdf','docforge_docx','docforge_pptx','docforge_xlsx']);
  var DOCFORGE_ROUTE = {
    'docforge_pdf':  '/office.html?embed=1&view=files&tool=pdf&pdfSave=vault&pdfMode=vault',
    'docforge_docx': '/office.html?embed=1&view=files&tool=docx',
    'docforge_pptx': '/office.html?embed=1&view=files&tool=pptx',
    'docforge_xlsx': '/office.html?embed=1&view=files&tool=xlsx'
  };
  ...
  function getTierForTool(id){
    try {
      if (window.PDFREALM_TIER && typeof window.PDFREALM_TIER.getTierForTool === 'function') {
        return window.PDFREALM_TIER.getTierForTool(id);
      }
    } catch (_) {}
    if (SECURE_IDS.has(id)) return 'secure';
    return 'standard';
  }

  function suiteForTool(id){
    id = String(id||'').trim();
    if (!id) return 'standard';
    if (SIGNREQS_IDS && SIGNREQS_IDS.has(id)) return 'signreqs';
    if (DOCFORGE_IDS.has(id)) return 'docforge';
    if (id === GOV_ID) return 'governance';
    if (SECURE_IDS.has(id)) return 'secure';
    var tier = String(getTierForTool(id) || 'standard');
    if (tier === 'premium') return 'premium';
    return 'standard';
  }

  var SIGNREQS_IDS = new Set(['signreq_create','signreq_list','signreq_sign','signreq_proof']);

  function toolMatchesToolset(id, toolset){
    id = String(id||'').trim();
    toolset = String(toolset||'').trim();
    if (!id || !toolset) return false;
    if (toolset === 'signreqs') return SIGNREQS_IDS.has(id);
    if (toolset === 'docforge') return DOCFORGE_IDS.has(id);
    if (toolset === 'secure') return SECURE_IDS.has(id);
    if (toolset === 'governance') return id === GOV_ID;
    if (toolset === 'premium') return suiteForTool(id) === 'premium';
    return suiteForTool(id) === 'standard';
  }

  function tagSidebarSuites(){
    var tabs = $$('.tool-tabs .tool-tab[data-tool]');
    for (var i=0;i<tabs.length;i++) {
      var t = tabs[i];
      var id = String(t.getAttribute('data-tool')||'').trim();
      if (!id) continue;

      if (id === GOV_ID) t.setAttribute('data-gov','1'); else t.removeAttribute('data-gov');
      if (DOCFORGE_IDS.has(id)) t.setAttribute('data-docforge','1'); else t.removeAttribute('data-docforge');

      var suite = suiteForTool(id);
      if (suite === 'premium') t.setAttribute('data-suite','premium');
      else if (suite === 'standard') t.setAttribute('data-suite','standard');
      else t.removeAttribute('data-suite');
    }
  }

  function buildMenu(tabEl, toolset){
    var menu = $('.toolset-menu', tabEl);
    if (!menu) return;
    menu.innerHTML = '';

    if (String(toolset||'').trim() === 'governance') {
      var mods = [
        { bt: 'hash',          label: 'Hash Evidence Machine' },
        { bt: 'timetravel',    label: 'PDF Time Travel™' },
        { bt: 'interrogation', label: 'PDF Interrogation Mode™' },
      ...
      gridG.className = 'toolset-menu-grid';
      try { gridG.style.gridTemplateColumns = '1fr'; } catch (_) {}

      for (var k=0;k<mods.length;k++) {
        var mm = mods[k];
        var itemG = document.createElement('button');
        itemG.type = 'button';
        itemG.className = 'toolset-item';
        itemG.setAttribute('role','menuitem');
        itemG.setAttribute('data-tool', GOV_ID);
        itemG.setAttribute('data-bt', mm.bt);

        var spanG = document.createElement('span');
        spanG.textContent = mm.label;
        itemG.appendChild(spanG);
        gridG.appendChild(itemG);
      }
      menu.appendChild(gridG);
      return;
    }

    var toolTabs = $$('.tool-tabs .tool-tab[data-tool]');
    ...
    for (var i=0;i<toolTabs.length;i++) {
      ...
      seen[id] = true;
      if (!toolMatchesToolset(id, toolset)) continue;
      matches.push(t);
    }
    ...
    for (var j=0;j<matches.length;j++) {
      var t2 = matches[j];
      var id2 = String(t2.getAttribute('data-tool')||'').trim();
      var label = normText(t2.textContent) || id2;

      var item = document.createElement('button');
      item.type = 'button';
      item.className = 'toolset-item';
      item.setAttribute('role','menuitem');
      item.setAttribute('data-tool', id2);
  ...
  function markActiveButtons(toolset){
    var bar = document.getElementById('toolsetTabs');
    if (!bar) return;
    var tabs = $$('.toolset-tab[data-toolset]', bar);
    for (var i=0;i<tabs.length;i++) {
      var t = tabs[i];
      var isOn = (t.getAttribute('data-toolset') || '') === toolset;
      var b = $('.toolset-btn', t);
      if (b) b.classList.toggle('active', !!isOn);
    }
  }

  function setActiveToolset(toolset){
    toolset = String(toolset||'').trim() || 'standard';
    if (!ALLOWED_TOOLSETS.has(toolset)) toolset = 'standard';
    try { localStorage.setItem(KEY, toolset); } catch (_) {}
    try { document.body && document.body.setAttribute('data-toolset', toolset); } catch (_) {}
    markActiveButtons(toolset);
  }

  function getSavedToolset(){
    try {
      var v = localStorage.getItem(KEY);
      if (v && ALLOWED_TOOLSETS.has(String(v))) return String(v);
    } catch (_) {}
    return 'standard';
  }

  function ensureActiveToolVisible(toolset){
    toolset = String(toolset||'').trim() || 'standard';
    if (toolset === 'governance') return; // GOV_NO_AUTOSELECT_V1
    try {
      var active = document.querySelector('.tool-tab.tool-tab-active[data-tool]');
      var activeId = active ? String(active.getAttribute('data-tool')||'').trim() : '';
      if (activeId && toolMatchesToolset(activeId, toolset)) return;

      var toolTabs = $$('.tool-tabs .tool-tab[data-tool]');
      for (var i=0;i<toolTabs.length;i++) {
        var id = String(toolTabs[i].getAttribute('data-tool')||'').trim();
        if (!id) continue;
        if (toolMatchesToolset(id, toolset)) {
          if (typeof toolTabs[i].click === 'function') toolTabs[i].click();
          return;
        }
      }
    } catch (_) {}
  ...
  function activateTool(toolId){
    ...
    if (!id) return;
    if (window.pdfTrack) pdfTrack('tool_selected', { toolName: id });
    if (DOCFORGE_IDS.has(id)) {
      setActiveToolset('docforge');
      try {
        var btnDf = document.querySelector('.tool-tab[data-tool="' + id.replace(/\"/g,'\\"') + '"]');
        if (btnDf && typeof btnDf.click === 'function') { btnDf.click(); return; }
      } catch (_) {}
      try { if (typeof window.__pdfrealmActivateTool === 'function') window.__pdfrealmActivateTool(id); } catch (_) {}
      return;
    }

    setActiveToolset(suiteForTool(id));

    try {
      var btn = document.querySelector('.tool-tab[data-tool="' + id.replace(/\"/g,'\\\"') + '"]');
  ...
  function wire(){
    var bar = document.getElementById('toolsetTabs');
    if (!bar) return;
    if (bar.__pdfrealmToolsetV3Wired) return;
    bar.__pdfrealmToolsetV3Wired = true;

    bar.addEventListener('click', function(e){
      // allow AI Operator to behave like a normal link
      try {
        if (e && e.target && e.target.closest && e.target.closest('#aiOperatorLink, a[href="/?ai=1"]')) return;
      } catch (_) {}

      var btn = e.target && e.target.closest ? e.target.closest('.toolset-btn') : null;
      var item = e.target && e.target.closest ? e.target.closest('.toolset-item[data-tool]') : null;

      if (btn && bar.contains(btn)) {
        e.preventDefault();
        e.stopPropagation();

        var tabEl = btn.closest('.toolset-tab[data-toolset]');
        if (!tabEl) return;

        var toolset = String(tabEl.getAttribute('data-toolset')||'standard').trim() || 'standard';
        if (!ALLOWED_TOOLSETS.has(toolset)) toolset = 'standard';
    ...
    injectGovModules();
        ...
        // For DocForge we DO want the dropdown (PDFStudio / DocX / PPTX / ExcelX) in the header.
        // Other suites keep the "no dropdown" behavior and rely on the left sidebar.
        if (toolset !== "docforge") {
          closeAll();
          ensureActiveToolVisible(toolset);
          return;
        }

        buildMenu(tabEl, toolset);

        var wasOpen = tabEl.classList.contains('open');
        closeAll();
        if (!wasOpen) {
      ...
      if (item && bar.contains(item)) {
        ...
        var bt = item.getAttribute('data-bt');

        activateTool(tool);

        if (bt) {
          setTimeout(function(){
            try { if (typeof window.__pdfrealmBTActivate === 'function') window.__pdfrealmBTActivate(bt); } catch (_) {}
          }, 0);
        }
        return;
      }
    ...
    document.addEventListener('click', function(e){
      try { if (bar && e && e.target && bar.contains(e.target)) return; } catch (_) {}
      closeAll();
    });

    // esc closes
    document.addEventListener('keydown', function(e){
      if (e && (e.key === 'Escape' || e.key === 'Esc')) closeAll();
    });

    // boot
    tagSidebarSuites();
    var saved = getSavedToolset();
    setActiveToolset(saved);
    ensureActiveToolVisible(saved);

    // Build all menus once (optional)
    setTimeout(function(){
      try {
        var tabs = $$('.toolset-tab[data-toolset]', bar);
        for (var i=0;i<tabs.length;i++) {
          buildMenu(tabs[i], String(tabs[i].getAttribute('data-toolset')||'standard'));
        }
      } catch (_) {}
    }, 150);
  }
...
/* GOV_MODULE_TAB_WIRE_V1 */
(function(){
  document.addEventListener('click', function(e){
    var btn = e.target && e.target.closest ? e.target.closest('.tool-tabs .tool-tab[data-tool="breakthrough"][data-bt]') : null;
    if (!btn) return;
    var bt = btn.getAttribute('data-bt');
    if (!bt) return;

    // Let the normal tool-tab handler activate "breakthrough", then select the module.
    setTimeout(function(){
      try { if (typeof window.__pdfrealmBTActivate === 'function') window.__pdfrealmBTActivate(bt); } catch (_) {}
    }, 0);
  }, true);
...
/* SAFE_BREAKTHROUGH_GUARD_V1 */
(function(){
  function getActiveToolId(){
    var a = document.querySelector('.tool-tabs .tool-tab.tool-tab-active[data-tool]');
    return a ? String(a.getAttribute('data-tool')||'').trim() : '';
  }
  function getToolset(){
    try { return String(document.body.getAttribute('data-toolset')||'').trim(); } catch(_) { return ''; }
  }
  function clickStudio(){
    var b = document.querySelector('.tool-tabs .tool-tab[data-tool="studio"]');
    if (b && typeof b.click === 'function') b.click();
  }
  function enforce(){
    var ts = getToolset();
    var id = getActiveToolId();
    if (id === 'breakthrough' && ts !== 'governance'){
      clickStudio();
      // extra: ensure breakthrough view is not active
      try{
        document.querySelectorAll('.tool-view.tool-view-active[data-tool-view="breakthrough"]').forEach(function(v){
          v.classList.remove('tool-view-active');
        });
      }catch(_){}
    }
  }
  ...
  function boot(){
    enforce();
    // only run on navigation clicks (no loops/observers)
    document.addEventListener('click', function(e){
      var t = e && e.target;
      if (!t || !t.closest) return;
      if (t.closest('#toolsetTabs, .tier-tabs, .tool-tabs')) setTimeout(enforce, 0);
    }, true);
    // and after initial render
...
/* ==================== PDFREALM_EVIDENCE_CORE_GUARD_V1 ====================
   Prevent "Evidence Core (Live) — Phase 1" from living in the LEFT sidebar.
   - If the panel exists, relocate it into #evidenceCoreMount (Governance -> Hash)
   - Hide it unless breakthrough view is active AND hash section is active
========================================================================= */
(function () {
  'use strict';
  if (window.__PDFREALM_EVIDENCE_CORE_GUARD_V1__) return;
  window.__PDFREALM_EVIDENCE_CORE_GUARD_V1__ = true;

  function $(sel, root){ return (root||document).querySelector(sel); }

  function getPanel(){
    return document.getElementById('evidenceCorePanel')
        || document.getElementById('evidenceCore')
        || document.getElementById('evidenceCoreCard')
        || $('[data-evidence-core]');
  }

  function getMount(){
    return document.getElementById('evidenceCoreMount')
        || $('.tool-view[data-tool-view="breakthrough"] .bt-section[data-bt-section="hash"]')
        || null;
  }

  function isGovHashActive(){
    var v = $('.tool-view.tool-view-active[data-tool-view="breakthrough"]');
    if (!v) return false;
    return !!$('.bt-section.bt-active[data-bt-section="hash"]', v);
  }
  ...
  function relocate(){
    ...
    // Never allow it to sit in the main sidebar
    try {
    ...
    var mount = getMount();
    if (mount && !mount.contains(panel)) {
      try { mount.appendChild(panel); } catch (_) {}
    }

    var show = isGovHashActive();
    try { panel.style.display = show ? '' : 'none'; } catch (_) {}
  }

  function schedule(){
    if (schedule.t) clearTimeout(schedule.t);
    schedule.t = setTimeout(relocate, 0);
  }
  ...
  // Clicks and suite/tool changes are enough; no interval loops
  document.addEventListener('click', schedule, true);

  // Watch toolset changes (header suite switches)
  try {
    var obs = new MutationObserver(function(){ schedule(); });
    obs.observe(document.body, { attributes:true, attributeFilter:['data-toolset'] });
  } catch (_) {}
})();
...
(function initAiChatWidget() {
  ...
  const creditDisplay = document.getElementById('ai-chat-credit-display');
  const creditBadge = document.getElementById('ai-chat-credit-badge');
  const creditsLow = document.getElementById('ai-chat-credits-low');
  const clearBtn = document.getElementById('ai-chat-clear-btn');
  const suggestions = document.querySelectorAll('.ai-suggestion');
  const buyLink = document.getElementById('ai-buy-credits-link');
  ...
  function getCurrentToolContext() {
    const active = document.querySelector('.tool-tab.tool-tab-active[data-tool]');
    return active ? active.getAttribute('data-tool') : 'default';
  }

  function updateCreditDisplay(c) {
    credits = c;
    if (creditDisplay) creditDisplay.textContent = c === null ? '' : `${c} credit${c !== 1 ? 's' : ''} remaining`;
    if (creditsLow) creditsLow.style.display = (c !== null && c <= 5) ? 'block' : 'none';
    if (creditBadge) {
      if (c !== null && c <= 0) { creditBadge.textContent = '0'; creditBadge.style.display = 'block'; }
      else creditBadge.style.display = 'none';
    }
  }

  function getAiToken() {
    try { return getAiToken(); } catch { return ''; }
  }

  async function loadStatus() {
    try {
      const r = await fetch('/api/ai-chat/status', { headers: { Authorization: 'Bearer ' + getAiToken() } });
      if (r.ok) { const d = await r.json(); updateCreditDisplay(d.credits); }
    } catch {}
  }
  ...
  toggle.addEventListener('click', () => {
    ...
    if (isOpen && credits === null) loadStatus();
  });
  ...
  clearBtn.addEventListener('click', async () => {
    await fetch('/api/ai-chat/clear', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + (getAiToken()) }, body: JSON.stringify({ sessionId }) });
    sessionId = 'session_' + Date.now();
    messagesEl.innerHTML = '<div class="ai-msg ai-msg-assistant"><div class="ai-msg-bubble">Chat cleared! Ask me anything. ✨</div></div>';
  });
  ...
  if (buyLink) {
    buyLink.addEventListener('click', async (e) => {
      e.preventDefault();
      const r = await fetch('/api/ai-chat/buy', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + (getAiToken()) }, body: JSON.stringify({ bundle: 'starter' }) });
  ...
  async function sendMessage(text) {
    ...
    if (credits !== null && credits <= 0) {
      addMessage('assistant', 'You\'re out of AI credits. Click "Buy more" below to continue chatting!');
      return;
    }
    isBusy = true;
    ...
    const sugEl = document.getElementById('ai-chat-suggestions');
    ...
    try {
      ...
      const r = await fetch('/api/ai-chat/message', {
        ...
        body: JSON.stringify({ message: text, sessionId, toolContext: getCurrentToolContext() })
      });
      const d = await r.json();
      typingEl.remove();

      if (r.status === 402) {
        addMessage('assistant', d.message || 'Out of credits. Purchase more to continue!');
        updateCreditDisplay(0);
      } else if (!r.ok) {
        addMessage('assistant', d.error || 'Something went wrong. Please try again.');
      } else {
        const msgEl = addMessage('assistant', d.reply);
        if (d.actionExecuted) {
          const statusEl = document.createElement('div');
          statusEl.style.cssText = 'font-size:0.75rem;color:#34d399;margin-top:4px;';
          statusEl.textContent = '⚡ Action executed';
          msgEl.querySelector('.ai-msg-bubble').appendChild(statusEl);
        }
        updateCreditDisplay(d.creditsRemaining);
      }
    } catch (e) {
      typingEl.remove();
  ...
  // Check credits on page load if user is logged in
  const token = localStorage.getItem('pdfrealm_token') || document.cookie.match(/pdfrealm_token=([^;]+)/)?.[1];
  if (token) setTimeout(loadStatus, 2000);
...
/* ============ APP DOWNLOAD MODAL ============ */
(function() {
  var btn = document.getElementById('appDownloadNavBtn');
  var modal = document.getElementById('appDownloadModal');
  var closeBtn = document.getElementById('appDownloadModalClose');
  if (!btn || !modal) return;
...
(function() {
  ...
  const STORAGE_KEY = 'pdfrealm_calendar';
  ...
  style.textContent = `
    #calendarDropdown {
      ...
      background:#1a1d23; border:1px solid rgba(255,255,255,0.13);
      border-radius:14px; box-shadow:0 8px 40px rgba(0,0,0,0.55);
      width:340px; padding:18px;
      display:none; flex-direction:column; gap:12px;
      font-family:system-ui,sans-serif; color:var(--text,#e2e8f0);
    }
    #calendarDropdown.cal-open { display:flex; }
    .cal-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:4px; }
    .cal-header h3 { margin:0; font-size:1rem; font-weight:600; }
    .cal-nav-btn { background:rgba(255,255,255,0.07); border:1px solid rgba(255,255,255,0.1);
      color:var(--text,#e2e8f0); border-radius:6px; padding:4px 10px; cursor:pointer; font-size:0.95rem; }
    .cal-nav-btn:hover { background:rgba(255,255,255,0.13); }
    .cal-grid { display:grid; grid-template-columns:repeat(7,1fr); gap:3px; }
    .cal-dow { text-align:center; font-size:0.72rem; color:#6b7280; padding:3px 0; font-weight:600; text-transform:uppercase; }
    .cal-day { text-align:center; padding:6px 2px; border-radius:6px; cursor:pointer; font-size:0.88rem;
      position:relative; transition:background 0.12s; min-height:30px; display:flex; flex-direction:column;
      align-items:center; justify-content:center; gap:2px; }
    .cal-day:hover { background:rgba(96,165,250,0.15); }
    .cal-day.today { background:rgba(96,165,250,0.22); font-weight:700; color:#60a5fa; }
    .cal-day.has-note::after { content:''; display:block; width:5px; height:5px; border-radius:50%;
      background:#60a5fa; margin:0 auto; }
    .cal-day.empty { cursor:default; }
    .cal-day.other-month { opacity:0.3; }
    .cal-form { background:rgba(255,255,255,0.04); border-radius:10px; padding:14px;
      display:flex; flex-direction:column; gap:10px; border:1px solid rgba(255,255,255,0.08); }
    .cal-form h4 { margin:0; font-size:0.92rem; color:#60a5fa; }
    .cal-form input, .cal-form textarea {
      background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12);
      color:var(--text,#e2e8f0); border-radius:7px; padding:8px 10px; font-size:0.88rem;
      width:100%; box-sizing:border-box; font-family:inherit; resize:none; }
    .cal-form input::placeholder, .cal-form textarea::placeholder { color:#6b7280; }
    .cal-form-row { display:flex; gap:8px; }
    .cal-form-row input { flex:1; }
    .cal-btn-row { display:flex; gap:8px; }
    .cal-btn { flex:1; padding:8px 12px; border-radius:7px; border:none; cursor:pointer;
      font-size:0.85rem; font-weight:600; transition:opacity 0.15s; }
    .cal-btn-save { background:#3b82f6; color:#fff; }
    .cal-btn-remind { background:#10b981; color:#fff; }
    .cal-btn-delete { background:rgba(239,68,68,0.2); color:#f87171; border:1px solid rgba(239,68,68,0.3); }
    .cal-btn:hover { opacity:0.85; }
    .cal-msg { font-size:0.82rem; padding:6px 10px; border-radius:6px; text-align:center; }
    .cal-msg.success { background:rgba(16,185,129,0.15); color:#6ee7b7; }
    .cal-msg.error { background:rgba(239,68,68,0.15); color:#fca5a5; }
    .cal-close-btn { background:none; border:none; color:#6b7280; cursor:pointer; font-size:1.1rem; padding:2px 6px; border-radius:4px; }
    .cal-close-btn:hover { color:#e2e8f0; }
    .cal-note-preview { font-size:0.85rem; color:#9ca3af; line-height:1.5; word-break:break-word; }
  `;
  document.head.appendChild(style);
  ...
  function renderCalendar() {
    ...
    let html = `
      <div class="cal-header">
        <h3>📅 Calendar</h3>
        <button class="cal-close-btn" id="calCloseBtn">✕</button>
      </div>
      <div class="cal-header">
    ...
    if (selectedDate) {
      ...
      html += `<div class="cal-form" id="calForm">
        <h4>${displayDate(selectedDate)}</h4>`;

      if (existing) {
        html += `
          <div class="cal-note-preview">${existing.title ? `<strong>${existing.title}</strong><br>` : ''}${existing.note}</div>
          <div class="cal-btn-row">
            <button class="cal-btn cal-btn-delete" id="calDeleteBtn">🗑 Delete</button>
            <button class="cal-btn cal-btn-save" id="calEditBtn" style="background:rgba(255,255,255,0.08);color:var(--text);">✏ Edit</button>
          </div>`;
      } else {
        html += `
          <input type="text" id="calTitle" placeholder="Title (optional)" maxlength="80" />
          <textarea id="calNote" placeholder="Note / reminder..." rows="3" maxlength="500"></textarea>
          <input type="email" id="calEmail" placeholder="Email for reminder (optional)" />
          <div class="cal-form-row">
            <input type="time" id="calTime" title="Reminder time (optional)" />
          </div>
          <div class="cal-btn-row">
            <button class="cal-btn cal-btn-save" id="calSaveBtn">💾 Save Note</button>
            <button class="cal-btn cal-btn-remind" id="calRemindBtn">📧 Send Reminder</button>
          </div>`;
      }

      html += `<div id="calMsg"></div></div>`;
    }
  ...
  function bindCalendarEvents() {
    ...
    if (prev) prev.addEventListener('click', () => { currentMonth--; if(currentMonth<0){currentMonth=11;currentYear--;} renderCalendar(); });
    if (next) next.addEventListener('click', () => { currentMonth++; if(currentMonth>11){currentMonth=0;currentYear++;} renderCalendar(); });
    if (closeBtn) closeBtn.addEventListener('click', closeCalendar);

    // Day clicks
    dropdown.querySelectorAll('.cal-day[data-date]').forEach(el => {
    ...
    if (saveBtn) saveBtn.addEventListener('click', function() {
      ...
      if (!note) { showCalMsg('Please enter a note.', 'error'); return; }
      const notes = loadNotes();
      notes[selectedDate] = { title, note, createdAt: Date.now() };
      saveNotes(notes);
      showCalMsg('Note saved!', 'success');
      setTimeout(renderCalendar, 800);
    });

    // Send reminder
    const remindBtn = document.getElementById('calRemindBtn');
    if (remindBtn) remindBtn.addEventListener('click', async function() {
      const title = (document.getElementById('calTitle').value || '').trim();
      const note = (document.getElementById('calNote').value || '').trim();
      const email = (document.getElementById('calEmail').value || '').trim();
      if (!note) { showCalMsg('Please enter a note.', 'error'); return; }
      if (!email) { showCalMsg('Please enter an email for the reminder.', 'error'); return; }

      // Also save locally
      const notes = loadNotes();
      notes[selectedDate] = { title, note, email, createdAt: Date.now() };
      saveNotes(notes);
      ...
      try {
        const res = await fetch('/api/calendar/remind', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email, date: selectedDate, note, title })
        });
        const data = await res.json();
        if (res.ok) {
          showCalMsg('✅ Reminder sent to ' + email, 'success');
          setTimeout(renderCalendar, 1500);
        } else {
          showCalMsg('❌ ' + (data.error || 'Failed to send'), 'error');
        }
      } catch(e) {
        showCalMsg('❌ Network error', 'error');
      } finally {
    ...
    // Edit → switch to edit mode (same as no-note)
    const editBtn = document.getElementById('calEditBtn');
  ...
  function showCalMsg(msg, type) {
    const el = document.getElementById('calMsg');
    if (!el) return;
    el.className = 'cal-msg ' + type;
    el.textContent = msg;
  }
  ...
  function closeCalendar() {
    dropdown.classList.remove('cal-open');
  }

  function toggleCalendar() {
    if (dropdown.classList.contains('cal-open')) {
      closeCalendar();
    } else {
      openCalendar();
    }
  ...
  document.addEventListener('click', function(e) {
    if (!dropdown.contains(e.target) && e.target.id !== 'calendarToggleBtn') {
      closeCalendar();
    }
  });
  ...
  window.addEventListener('resize', function() {
    if (dropdown.classList.contains('cal-open')) positionDropdown();
  });

  // Init on DOM ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCalendarBtn);
  } else {
    initCalendarBtn();
  }

  // Also try after a short delay (for pages that load content dynamically)
  setTimeout(initCalendarBtn, 500);
...
/* ============================================================
   PDFREALM_EMAIL_FEATURES_V2
   Feature 1: Email PDF after quicktool processing
   Feature 2: Email secure share links
   ============================================================ */
(function () {
  'use strict';
  if (window.__pdfrealm_email_v2) return;
  window.__pdfrealm_email_v2 = true;

  /* ---- Global state ---- */
  window.__emailPdfState  = { blob: null, filename: 'processed.pdf', toolName: 'Processed' };
  window.__emailShareState = { shareId: null, shareUrl: null };

  /* ---- Patch runPaidPdfExport to capture the last blob ---- */
  const _orig = window.runPaidPdfExport;
  if (typeof _orig === 'function') {
    window.runPaidPdfExport = async function (opts) {
      const result = await _orig(opts);
      if (result && result.ok && result.blob) {
        window.__emailPdfState.blob     = result.blob;
        window.__emailPdfState.filename = opts.filename || 'processed.pdf';
        window.__emailPdfState.toolName = String(opts.tool || 'Processed')
          .replace(/-/g, ' ')
  ...
  /* ---- Patch qtFetchJson to capture share creation ---- */
  const _origFetch = window.qtFetchJson;
  if (typeof _origFetch === 'function') {
    window.qtFetchJson = async function (url, opts) {
      const result = await _origFetch(url, opts);
      if (result && result.id && (result.share_url || result.url)) {
        window.__emailShareState.shareId  = result.id;
        window.__emailShareState.shareUrl = result.share_url || result.url || '';
      }
      return result;
    };
  }
  ...
  /* ---- Floating "Email this PDF" CTA (appears after any export) ---- */
  function showEmailCta(show) {
    let btn = document.getElementById('__emailPdfCta');
    if (!btn) {
      btn = document.createElement('button');
      btn.id = '__emailPdfCta';
      btn.className = 'btn btn-secondary';
      btn.type = 'button';
      btn.innerHTML = '\u2709 Email this PDF';
      btn.title = 'Email your processed PDF to an address';
      Object.assign(btn.style, {
        position: 'fixed', bottom: '22px', right: '22px', zIndex: '9100',
        display: 'none', boxShadow: '0 4px 18px rgba(0,0,0,0.18)',
        padding: '10px 22px', fontSize: '0.93rem', borderRadius: '8px',
        cursor: 'pointer'
      });
      btn.addEventListener('click', () => {
        if (!window.__emailPdfState.blob) { alert('No PDF available. Export a file first.'); return; }
        const inp = document.getElementById('emailPdfAddress');
        if (inp) inp.value = '';
        setStatus('emailPdfStatus', '');
  ...
  /* ---- Feature 1: Email PDF modal ---- */
  function wireEmailPdfModal() {
    const modal     = document.getElementById('emailPdfModal');
    if (!modal) return;
    const closeBtn  = document.getElementById('emailPdfModalClose');
    const cancelBtn = document.getElementById('emailPdfCancelBtn');
    const sendBtn   = document.getElementById('emailPdfSendBtn');
    const emailInp  = document.getElementById('emailPdfAddress');

    [closeBtn, cancelBtn].forEach(b => b && b.addEventListener('click', () => closeModal('emailPdfModal')));
    modal.addEventListener('click', e => { if (e.target === modal) closeModal('emailPdfModal'); });

    sendBtn && sendBtn.addEventListener('click', async () => {
      const email = (emailInp ? emailInp.value : '').trim();
      if (!email || !email.includes('@')) { setStatus('emailPdfStatus', 'Please enter a valid email.', true); return; }

      const { blob, filename, toolName } = window.__emailPdfState;
      if (!blob) { setStatus('emailPdfStatus', 'No PDF to send. Export a file first.', true); return; }

      sendBtn.disabled = true;
      setStatus('emailPdfStatus', 'Sending\u2026');
      ...
      try {
        ...
        const headers = {};
        if (token) headers['Authorization'] = 'Bearer ' + token;
        const ppeKey = typeof getPpeForTool === 'function'
          ? getPpeForTool(String(toolName).toLowerCase().replace(/\s+/g, '-')) : '';
        if (ppeKey) headers['x-ppe-session'] = ppeKey;

        const res  = await fetch('/api/email-pdf', { method: 'POST', headers, body: fd });
        const data = await res.json().catch(() => ({}));

        if (data.ok && !data.emailFailed) {
          setStatus('emailPdfStatus', '\u2713 Sent! Check your inbox.');
          if (emailInp) emailInp.value = '';
          setTimeout(() => closeModal('emailPdfModal'), 2600);
        } else {
          setStatus('emailPdfStatus', 'Email delivery failed — please try again.', true);
        }
      } catch (err) {
        setStatus('emailPdfStatus', 'Error: ' + (err.message || err), true);
      } finally {
        sendBtn.disabled = false;
      }
    });
  ...
  /* ---- Feature 2: Email secure link modal ---- */
  function wireEmailShareModal() {
    const modal     = document.getElementById('emailShareModal');
    if (!modal) return;
    const closeBtn  = document.getElementById('emailShareModalClose');
    const cancelBtn = document.getElementById('emailShareCancelBtn');
    const sendBtn   = document.getElementById('emailShareSendBtn');
    const emailInp  = document.getElementById('emailShareAddress');
    const msgInp    = document.getElementById('emailShareMessage');

    [closeBtn, cancelBtn].forEach(b => b && b.addEventListener('click', () => closeModal('emailShareModal')));
    modal.addEventListener('click', e => { if (e.target === modal) closeModal('emailShareModal'); });

    sendBtn && sendBtn.addEventListener('click', async () => {
      const email = (emailInp ? emailInp.value : '').trim();
      if (!email || !email.includes('@')) { setStatus('emailShareStatus', 'Please enter a valid recipient email.', true); return; }

      const { shareId, shareUrl } = window.__emailShareState;
      if (!shareId) { setStatus('emailShareStatus', 'No share selected. Create a secure link first.', true); return; }

      sendBtn.disabled = true;
      setStatus('emailShareStatus', 'Sending\u2026');
      ...
      try {
        ...
        const res = await fetch('/api/secure-shares/' + encodeURIComponent(shareId) + '/email', {
          method: 'POST', headers,
          body: JSON.stringify({ email, message: msgInp ? msgInp.value.trim() : '', shareUrl: shareUrl || '' })
        });
        const data = await res.json().catch(() => ({}));

        if (data.ok && !data.emailFailed) {
          setStatus('emailShareStatus', '\u2713 Link sent! Check their inbox.');
          if (emailInp) emailInp.value = '';
          if (msgInp)   msgInp.value   = '';
          setTimeout(() => closeModal('emailShareModal'), 2600);
        } else {
          setStatus('emailShareStatus', 'Email delivery failed — please try again.', true);
        }
      } catch (err) {
        setStatus('emailShareStatus', 'Error: ' + (err.message || err), true);
      } finally {
        sendBtn.disabled = false;
      }
    });
  ...
  /* ---- "✉ Email Link" button in Secure Send output ---- */
  function wireQtSendEmailBtn() {
    const btn = document.getElementById('qtSendEmailLinkBtn');
    if (!btn) return;
    btn.addEventListener('click', () => {
      const linkInp  = document.getElementById('qtSendLink');
      const shareUrl = linkInp ? linkInp.value.trim() : '';
      if (!shareUrl || !shareUrl.includes('/s/')) { alert('Please create a secure link first.'); return; }
      // Update state (in case qtFetchJson patch didn't capture it)
      window.__emailShareState.shareUrl = shareUrl;
      const emailInp = document.getElementById('emailShareAddress');
      const msgInp   = document.getElementById('emailShareMessage');
      if (emailInp) emailInp.value = '';
      if (msgInp)   msgInp.value   = '';
      setStatus('emailShareStatus', '');
      openModal('emailShareModal');
    });
  }
...
/* ============ END PDFREALM_EMAIL_FEATURES_V2 ============ */
