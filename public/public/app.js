...
function downloadBlob(blob, filename) {
  ...
  // Show a floating "Save to Vault" button after download
  showSaveToVaultButton(blob, filename || "output.pdf");
  return url;
}

// -------------------- Save-to-Vault helper --------------------
// Appends a temporary "💾 Save to Vault" button near the most recently clicked export button,
// or in a fixed notification if no context element is found.
function showSaveToVaultButton(blob, filename) {
  // Remove any existing save-to-vault buttons
  document.querySelectorAll(".pdfrealm-save-vault-btn-wrap").forEach(el => el.remove());

  const wrap = document.createElement("div");
  wrap.className = "pdfrealm-save-vault-btn-wrap";
  ...
  btn.style.cssText = "font-size:14px;padding:10px 18px;border-radius:8px;box-shadow:0 2px 12px rgba(0,0,0,0.18);cursor:pointer;";

  const statusEl = document.createElement("span");
  statusEl.style.cssText = "font-size:12px;color:#374151;background:#fff;padding:4px 10px;border-radius:6px;box-shadow:0 1px 6px rgba(0,0,0,0.1);display:none;";

  btn.addEventListener("click", async () => {
    btn.disabled = true;
    btn.textContent = "Saving…";
    statusEl.style.display = "inline";
    statusEl.textContent = "Uploading to vault…";
    try {
      const token = (typeof getToken === "function") ? getToken() : "";
      const fd = new FormData();
      fd.append("file", blob, filename);
      fd.append("folderPath", "Tool Outputs");
  ...
  closeBtn.style.cssText = "background:none;border:none;cursor:pointer;font-size:16px;color:#94a3b8;position:absolute;top:4px;right:6px;line-height:1;";
  closeBtn.addEventListener("click", () => wrap.remove());
  wrap.style.position = "fixed";

  const inner = document.createElement("div");
  inner.style.cssText = "position:relative;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:12px 16px;box-shadow:0 4px 20px rgba(0,0,0,0.15);";
...
async function saveToVault(blob, filename, folder) {
  const token = (typeof getToken === "function") ? getToken() : "";
  const fd = new FormData();
  fd.append("file", blob, filename || "output.pdf");
  fd.append("folderPath", folder || "Tool Outputs");
  const headers = {};
  if (token) headers["Authorization"] = "Bearer " + token;
  const res = await fetch("/api/vault/upload", { method: "POST", headers, body: fd });
  const data = await res.json();
  if (!data.ok) throw new Error(data.error || "Vault upload failed");
  return data;
}
...
var DOCFORGE_IDS = new Set(['docforge_pdf','docforge_docx','docforge_pptx','docforge_xlsx']);
  ...
  function buildMenu(tabEl, toolset){
    ...
    if (String(toolset||'').trim() === 'governance') {
      var mods = [
        { bt: 'hash',          label: '1️⃣ Hash Evidence Machine' },
        { bt: 'timetravel',    label: '2️⃣ PDF Time Travel™' },
        { bt: 'interrogation', label: '3️⃣ PDF Interrogation Mode™' },
        { bt: 'constitution',  label: '4️⃣ Document Constitution' },
        { bt: 'sentience',     label: '5️⃣ PDF Sentience Mode™' },
        { bt: 'immortal',      label: '6️⃣ Immortal Documents™' },
        { bt: 'moat',          label: '7️⃣ Evidence Graph & API' }
      ];
  ...
  function ensureActiveToolVisible(toolset){
    ...
    try {
      // Always activate the first matching tool so the view is always shown
      var toolTabs = $$('.tool-tabs .tool-tab[data-tool]');
      for (var i=0;i<toolTabs.length;i++) {
  ...
  function wire(){
    ...
    // Tag sidebar suites on init so data-suite attrs are ready before first click
    try { tagSidebarSuites(); } catch (_) {}
    try { injectGovModules(); } catch (_) {}
    ...
    injectGovModules();
        ...
        // All toolsets use the left sidebar — no header dropdowns
        closeAll();
...
// ============================================================
// PDFREALM_EMAIL_FEATURES_V1
// Feature 1: Email PDF after quicktool processing
// Feature 2: Email secure share links
// ============================================================
(function () {
  if (window.__pdfrealm_email_features_v1) return;
  window.__pdfrealm_email_features_v1 = true;

  // ---- Global state for last exported PDF blob ----
  window.__pdfEmailState = {
    blob: null,
    filename: 'processed.pdf',
    toolName: 'Processed'
  };

  // ---- Patch runPaidPdfExport to capture last blob ----
  const _origRunPaidPdfExport = window.runPaidPdfExport;
  if (typeof _origRunPaidPdfExport === 'function') {
    window.runPaidPdfExport = async function (opts) {
      const result = await _origRunPaidPdfExport(opts);
      if (result && result.ok && result.blob) {
        window.__pdfEmailState.blob = result.blob;
        window.__pdfEmailState.filename = opts.filename || 'processed.pdf';
        window.__pdfEmailState.toolName = String(opts.tool || 'Processed')
          .replace(/-/g, ' ')
          .replace(/\b\w/g, c => c.toUpperCase());
        // Show the email CTA button if it exists
        const cta = document.getElementById('emailPdfCta');
  ...
  // ---- Modal helpers ----
  function openEmailModal(modalId) {
    const el = document.getElementById(modalId);
    if (el) { el.setAttribute('aria-hidden', 'false'); el.style.display = ''; }
  }
  ...
  function setEmailStatus(statusId, msg, isErr) {
  ...
  function wireEmailPdfModal() {
    ...
    const emailInput = document.getElementById('emailPdfAddress');

    [closeBtn, cancelBtn].forEach(btn => {
      btn && btn.addEventListener('click', () => closeEmailModal('emailPdfModal'));
    });

    // Close on backdrop click
    modal.addEventListener('click', (e) => {
      if (e.target === modal) closeEmailModal('emailPdfModal');
    });

    sendBtn && sendBtn.addEventListener('click', async () => {
      const email = (emailInput ? emailInput.value : '').trim();
      if (!email || !email.includes('@')) {
        setEmailStatus('emailPdfStatus', 'Please enter a valid email address.', true);
        return;
      }
      const blob = window.__pdfEmailState.blob;
      if (!blob) {
        setEmailStatus('emailPdfStatus', 'No PDF to send. Please process a file first.', true);
        return;
      }

      sendBtn.disabled = true;
      setEmailStatus('emailPdfStatus', 'Sending…');

      try {
        const fd = new FormData();
        const filename = window.__pdfEmailState.filename || 'processed.pdf';
        const toolName = window.__pdfEmailState.toolName || 'Processed';
        fd.append('file', blob, filename);
        fd.append('email', email);
        fd.append('toolName', toolName);

        const token = typeof getToken === 'function' ? getToken() : '';
        const ppeSession = typeof getPpeForTool === 'function'
          ? (getPpeForTool(toolName.toLowerCase().replace(/\s+/g, '-')) || '')
          : '';
        const headers = {};
        if (token) headers['Authorization'] = `Bearer ${token}`;
        if (ppeSession) headers['x-ppe-session'] = ppeSession;
        ...
        if (data && data.ok && !data.emailFailed) {
          setEmailStatus('emailPdfStatus', '✓ Sent! Check your inbox.');
          emailInput && (emailInput.value = '');
          setTimeout(() => closeEmailModal('emailPdfModal'), 2500);
        } else if (data && data.ok && data.emailFailed) {
          setEmailStatus('emailPdfStatus', 'Email delivery failed. Please try again.', true);
        } else {
          setEmailStatus('emailPdfStatus', 'Failed to send. Please try again.', true);
        }
      } catch (err) {
        setEmailStatus('emailPdfStatus', 'Error: ' + (err.message || err), true);
      } finally {
  ...
  // State for the current share to email
  window.__pdfShareEmailState = { shareId: null, shareUrl: null };
  ...
  function wireEmailShareModal() {
    ...
    const emailInput = document.getElementById('emailShareAddress');
    const msgInput = document.getElementById('emailShareMessage');

    [closeBtn, cancelBtn].forEach(btn => {
      btn && btn.addEventListener('click', () => closeEmailModal('emailShareModal'));
    });
    modal.addEventListener('click', (e) => {
      if (e.target === modal) closeEmailModal('emailShareModal');
    });

    sendBtn && sendBtn.addEventListener('click', async () => {
      const email = (emailInput ? emailInput.value : '').trim();
      if (!email || !email.includes('@')) {
        setEmailStatus('emailShareStatus', 'Please enter a valid recipient email.', true);
        return;
      }
      const { shareId, shareUrl } = window.__pdfShareEmailState;
      if (!shareId) {
        setEmailStatus('emailShareStatus', 'No share selected. Please create a share first.', true);
        return;
      }

      sendBtn.disabled = true;
      setEmailStatus('emailShareStatus', 'Sending…');
      ...
      try {
        ...
        if (data && data.ok && !data.emailFailed) {
          setEmailStatus('emailShareStatus', '✓ Link sent! Check their inbox.');
          emailInput && (emailInput.value = '');
          msgInput && (msgInput.value = '');
          setTimeout(() => closeEmailModal('emailShareModal'), 2500);
        } else if (data && data.ok && data.emailFailed) {
          setEmailStatus('emailShareStatus', 'Email delivery failed. Please try again.', true);
        } else {
          setEmailStatus('emailShareStatus', 'Failed to send. Please try again.', true);
        }
      } catch (err) {
        setEmailStatus('emailShareStatus', 'Error: ' + (err.message || err), true);
      } finally {
        sendBtn.disabled = false;
      }
    });
  ...
  // ---- Wire "Email Link" button in Secure Send output area ----
  function wireQtSendEmailBtn() {
    const btn = document.getElementById('qtSendEmailLinkBtn');
    if (!btn) return;

    btn.addEventListener('click', () => {
      const linkInput = document.getElementById('qtSendLink');
      const shareUrl = linkInput ? linkInput.value.trim() : '';
      if (!shareUrl || shareUrl === '(link will appear here)') {
        alert('Please create a secure link first.');
        return;
      }

      // We don't have the shareId easily here; try to extract from URL or use a stored state.
      // The initSecureSend function stores the ID in created.id.
      // We'll read from our patched state if available, otherwise use URL-less approach.
      const shareId = window.__pdfShareEmailState && window.__pdfShareEmailState.shareId
        ? window.__pdfShareEmailState.shareId
        : 'unknown';

      window.__pdfShareEmailState.shareUrl = shareUrl;
      // Reset email/message fields
      const emailInput = document.getElementById('emailShareAddress');
      const msgInput = document.getElementById('emailShareMessage');
      if (emailInput) emailInput.value = '';
      if (msgInput) msgInput.value = '';
      setEmailStatus('emailShareStatus', '');
      openEmailModal('emailShareModal');
    });
  }

  // ---- Patch initSecureSend to capture share ID ----
  function patchSecureSendCapture() {
    // We intercept the qtFetchJson call for /api/secure-shares by watching for share creation.
    // The simplest approach: observe changes to the link input field.
    const linkInput = document.getElementById('qtSendLink');
    if (!linkInput) return;

    const observer = new MutationObserver(() => {});
    // Use input event and property setter monitoring via polling on value change
    let lastVal = '';
    const check = setInterval(() => {
      const val = linkInput.value || '';
      if (val !== lastVal && val.includes('/s/')) {
        lastVal = val;
        window.__pdfShareEmailState.shareUrl = val;
        // Try to get shareId from last API response — we'll also patch qtFetchJson
      }
    }, 500);
  }

  // ---- Patch qtFetchJson to capture share creation response ----
  const _origQtFetchJson = window.qtFetchJson;
  if (typeof _origQtFetchJson === 'function') {
    window.qtFetchJson = async function (url, opts) {
      const result = await _origQtFetchJson(url, opts);
      if (result && result.id && result.share_url) {
        window.__pdfShareEmailState.shareId = result.id;
        window.__pdfShareEmailState.shareUrl = result.share_url || result.url || '';
      }
      return result;
    };
  }

  // ---- Add global "✉ Email this PDF" button near tool export areas ----
  // We inject a floating "Email last PDF" CTA that appears after any export
  function injectEmailPdfCta() {
    // Avoid double inject
    if (document.getElementById('emailPdfCta')) return;

    const cta = document.createElement('button');
    cta.id = 'emailPdfCta';
    cta.className = 'btn btn-secondary';
    cta.type = 'button';
    cta.textContent = '✉ Email this PDF';
    cta.title = 'Email your processed PDF to an address';
    cta.style.cssText = [
      'position:fixed',
      'bottom:24px',
      'right:24px',
    ...
    cta.addEventListener('click', () => {
      if (!window.__pdfEmailState.blob) {
        alert('No PDF available. Please export a file first.');
        return;
      }
      const emailInput = document.getElementById('emailPdfAddress');
      if (emailInput) emailInput.value = '';
      setEmailStatus('emailPdfStatus', '');
      openEmailModal('emailPdfModal');
    });
