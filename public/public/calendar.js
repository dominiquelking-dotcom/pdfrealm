/**
 * PDFRealm Universal Calendar Widget
 * Injects calendar modal + wires toggle button on any page.
 * Uses localStorage key: pdfrealm.calendar.v1
 * Email reminders posted to /api/calendar/remind (Resend)
 */
(function () {
  'use strict';

  var CAL_KEY = 'pdfrealm.calendar.v1';

  // ── CSS ──────────────────────────────────────────────────────────────
  var style = document.createElement('style');
  style.textContent = [
    '.cal-modal{display:none;position:fixed;inset:0;z-index:9999;}',
    ...
    '.cal-topbar{',
      ...
      'background:rgba(255,255,255,0.02);gap:10px;',
    '}',
    ...
    '.cal-body{display:grid;grid-template-columns:minmax(0,1.6fr) minmax(0,1fr);gap:0;height:100%;}',
    '.cal-grid-wrap{border-right:1px solid var(--line);display:flex;flex-direction:column;min-width:0;}',
    '.cal-weekdays{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border-bottom:1px solid var(--line);background:rgba(255,255,255,0.02);}',
    '.cal-weekday{padding:10px 10px;font-size:0.86rem;color:var(--muted);border-right:1px solid rgba(255,255,255,0.06);}',
    '.cal-weekday:last-child{border-right:none;}',
    '.cal-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));grid-auto-rows:92px;flex:1;min-height:0;}',
    '.cal-day{border-right:1px solid rgba(255,255,255,0.06);border-bottom:1px solid rgba(255,255,255,0.06);padding:8px 8px;cursor:pointer;background:transparent;min-width:0;}',
    ...
    '.cal-dot{width:8px;height:8px;border-radius:999px;background:rgba(106,168,255,0.9);box-shadow:0 0 0 2px rgba(106,168,255,0.15);}',
    '.cal-items{margin-top:8px;display:flex;flex-direction:column;gap:6px;}',
    '.cal-item{border:1px solid rgba(106,168,255,0.35);background:rgba(106,168,255,0.10);color:var(--text);border-radius:10px;padding:6px 8px;font-size:0.86rem;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;}',
    ...
    '.cal-event-row{border:1px solid var(--line);border-radius:14px;background:rgba(255,255,255,0.03);padding:10px;margin-bottom:10px;}',
  ...
  // ── HTML (only inject if not already in page) ─────────────────────────
          ...
          '<div class="cal-body">',
            ...
            '<div class="cal-side">',
              ...
              '<div class="cal-form">',
                ...
                '<input id="calEventTitle" type="text" placeholder="Appointment, deadline, reminder..." />',
                ...
                '<textarea id="calEventNotes" rows="3" placeholder="Add details..."></textarea>',
                '<div class="cal-mini">',
                  '<h3 style="margin:0;font-size:1.02rem;">Email Reminder</h3>',
                  '<p style="margin-top:6px;color:var(--muted);font-size:0.92rem;">A confirmation email will be sent when you save an event with an email address.</p>',
  ...
  // ── Email reminder via backend ───────────────────────────────────────
  function sendEmailReminder(ev) {
    if (!ev.remindEmail) return;
    try {
      fetch('/api/calendar/remind', {
        ...
        body: JSON.stringify({
          ...
          time: ev.time || '',
          notes: ev.notes || '',
          remindEmail: ev.remindEmail,
          remindMinutes: ev.remindMinutes || ''
        })
      }).catch(function () {});
    } catch (e) {}
  }
  ...
  // Wire the header toggle button (may exist already or be added later)
  function wireToggleBtn() {
    var btn = document.getElementById('calendarToggleBtn');
    if (!btn || btn._calWired) return;
    btn._calWired = true;
  ...
  function getMonthGridStart(y2, m2) {
    ...
    var start = new Date(y2, m2, 1 - first.getDay());
  ...
  function renderSidePanel() {
    ...
    dayEvents.forEach(function (ev) {
      ...
      editBtn.onclick = (function (evRef) {
        return function () {
          state.editingId = evRef.id;
          if (calEventTitle) calEventTitle.value = evRef.title || '';
          if (calEventType) calEventType.value = evRef.type || 'appointment';
          if (calEventDate) calEventDate.value = evRef.date || state.selectedDate;
          if (calEventTime) calEventTime.value = evRef.time || '';
          if (calEventNotes) calEventNotes.value = evRef.notes || '';
          if (calRemindMinutes) calRemindMinutes.value = evRef.remindMinutes || '';
          if (calRemindEmail) calRemindEmail.value = evRef.remindEmail || '';
          if (calRemindPhone) calRemindPhone.value = evRef.remindPhone || '';
      ...
      delBtn.onclick = (function (evRef) {
        return function () {
          var all = loadEvents();
          saveEvents(all.filter(function (x) { return x.id !== evRef.id; }));
          if (state.editingId === evRef.id) state.editingId = null;
  ...
  if (calSaveBtn) calSaveBtn.addEventListener('click', function (e) {
    e.stopPropagation();
    var title = (calEventTitle ? calEventTitle.value : '').trim();
    var type = calEventType ? calEventType.value : 'appointment';
    var date = (calEventDate ? calEventDate.value : '') || state.selectedDate;
    var time = calEventTime ? calEventTime.value : '';
    var notes = (calEventNotes ? calEventNotes.value : '').trim();
    var remindMinutes = calRemindMinutes ? calRemindMinutes.value : '';
    var remindEmail = ((calRemindEmail ? calRemindEmail.value : '') || '').trim();
    var remindPhone = ((calRemindPhone ? calRemindPhone.value : '') || '').trim();
    ...
    // Send email reminder confirmation if email provided
    if (remindEmail) {
      sendEmailReminder(ev);
      if (calStatus) calStatus.textContent = (state.editingId ? 'Saved changes.' : 'Event saved.') + ' Email reminder sent to ' + remindEmail;
    }

    state.selectedDate = date;
    var parts2 = date.split('-');
    state.viewYear = Number(parts2[0]);
    state.viewMonth = Number(parts2[1]) - 1;
    state.editingId = null;
  ...
  function reminderTick() {
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      if (shouldTriggerReminder(ev, now)) {
        var msg = (ev.title || 'Event') + ' (' + ev.type + ') at ' + (ev.time || '09:00') + ' on ' + ev.date +
          (ev.notes ? (' — ' + ev.notes) : '');
  ...
  (function init() {
    ...
    // Wire toggle button (it exists in DOM now)
    wireToggleBtn();

    // Also wire any legacy buttons on these pages
    var legacyIds = ['legalCalendarBtn', 'brokerCalendarBtn', 'carrierCalendarBtn'];
    legacyIds.forEach(function (id) {
      var btn = document.getElementById(id);
      if (btn && !btn._calWired) {
        btn._calWired = true;
        btn.addEventListener('click', function (e) {
          e.stopPropagation();
          ensureNotifyPermission();
