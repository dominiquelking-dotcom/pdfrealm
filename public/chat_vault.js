...
(() => {
  ...
  const elEmailInviteBtn = $("chatEmailInviteBtn");
...
async function refreshParticipants({ silent = false } = {}) {
  ...
  async function generateInvite() {
    ...
    if (elEmailInviteBtn && url) {
      elEmailInviteBtn.href = `mailto:?subject=Secure Chat Invite&body=Join my secure chat: ${encodeURIComponent(url)}`;
      elEmailInviteBtn.style.pointerEvents = "auto";
      elEmailInviteBtn.style.opacity = "1";
    }
    syncButtons();
    await copyText(url);
